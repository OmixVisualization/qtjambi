...
public final class QtJambiMethodHandles implements QtJambiInternal.MethodInvocationHandler{
	public final boolean useMethodHandleProxies;

	QtJambiMethodHandles() {
	...
	public <A,B> Function<A,B> functionFromMethod(Method method){
		if(method==null)
			return null;
		try {
			Lookup lookup = QtJambiInternal.privateLookup(method.getDeclaringClass());
	...
	@Override
	public QtJambiSignals.SlotInvoker getSlotInvoker(Method slot, MethodHandle _slotHandle){
		if(_slotHandle==null) {
        	try {
				_slotHandle = QtJambiInternal.getMethodHandle(slot);
			} catch (Exception e) {
			}
        }
