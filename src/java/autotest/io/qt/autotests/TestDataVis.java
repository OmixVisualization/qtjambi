...
public class TestDataVis extends QApplicationTest {
    ...
    public void testQCustom3DVolume() {
    	...
    	Object textureData = volume.createTextureData(Arrays.asList(image1, image2));
    	assertEquals(Ownership.Split, ownership((QtObject)textureData));
    	assertFalse(((QtObject)textureData).isDisposed());
    	volume.dispose();
    	assertTrue(((QtObject)textureData).isDisposed());
    }
    ...
    public void testQBarDataProxy(){
    	...
    	assertEquals(Ownership.Cpp, ownership(array));
