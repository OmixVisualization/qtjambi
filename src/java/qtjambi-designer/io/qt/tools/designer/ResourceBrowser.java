...
package io.qt.tools.designer;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import io.qt.Utilities;
...
import io.qt.qtjambi.util.RetroTranslatorHelper;
...
public class ResourceBrowser extends JambiResourceBrowser {
    ...
    public ResourceBrowser(QWidget parent) {
        ...
        QSizePolicy policy = new QSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred);
            ...
            if (!UNFILTERED) {
            filterEdit.textChanged.connect(filterModel::setFilterRegExp);
        ...
        selection.currentChanged.connect(this::selectionChanged);

            setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu);
            addAction("Refresh", this::reindex);
            addAction("Edit searchpath", this::changeSearchPath);
    ...
    public void updateRootDirs(String paths) {
        ...
        if (roots != null) {
            for (String root : roots)
            	io.qt.core.QAbstractFileEngine.removeSearchPathForResourceEngine(root);
        }

        roots = new ArrayList<String>();
        Collections.addAll(roots, rootArray);
        for (String root : roots)
        	io.qt.core.QAbstractFileEngine.addSearchPathForResourceEngine(root);
    ...
    private void addAction(String text, Slot0 method) {
    ...
    private void setupSearchConnections() {
        ...
        browserModel.rowsAdded.connect(this::expand);
            ...
            // Add new roots to ClassPathFileEngine
            {
                for (String path : newPaths) {
                    if (!oldRoots.contains(path)) {
                    	io.qt.core.QAbstractFileEngine.addSearchPathForResourceEngine(path);
                    }
                }
            }
