...
class SplashScreen extends QSplashScreen {
    ...
    public SplashScreen() {
        ...
        final QColor dark = new QColor(io.qt.core.Qt.GlobalColor.black);
        final QColor light = new QColor(io.qt.core.Qt.GlobalColor.transparent);
        ...
        if (io.qt.QSysInfo.macVersion() > 0) {
            QBitmap bm = new QBitmap(tr.size().toSize());
            bm.fill(new QColor(io.qt.core.Qt.GlobalColor.color0));
            QPainter bmp = new QPainter(bm);
            bmp.translate(-tr.x(), -tr.y());
            bmp.fillPath(path, new QBrush(new QColor(io.qt.core.Qt.GlobalColor.color1)));
            bmp.end();
            setMask(bm);
        }
    }
