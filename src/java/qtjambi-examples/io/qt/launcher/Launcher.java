...
public class Launcher extends QWidget {
    ...
    private static String styleForCurrentSystem() {
        int os = io.qt.QSysInfo.operatingSystem();
        if (os == io.qt.QSysInfo.OS_WIN32
                || os == io.qt.QSysInfo.OS_WIN64) {
            if (io.qt.QSysInfo.windowsVersion() >= io.qt.QSysInfo.Windows_VISTA)
                return "WindowsVista";
            if (io.qt.QSysInfo.windowsVersion() >= io.qt.QSysInfo.Windows_XP)
                return "WindowsXP";
            else
                return "Windows";
        } else if (io.qt.QSysInfo.macVersion() > 0) {
