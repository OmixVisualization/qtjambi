...
package io.qt.demos.imageviewer;

import io.qt.launcher.Worker;
...
public class View extends QWidget
{
    ...
    public View(QWidget parent) {
        ...
        bg.fill(new QColor(io.qt.core.Qt.GlobalColor.white));
        QPainter p = new QPainter();
        p.begin(bg);
        p.fillRect(0, 0, size/2, size/2, new QBrush(new QColor(io.qt.core.Qt.GlobalColor.lightGray)));
        p.fillRect(size/2, size/2, size/2, size/2, new QBrush(new QColor(io.qt.core.Qt.GlobalColor.lightGray)));
        p.end();
    ...
    protected void paintEvent(QPaintEvent e) {
        if (background == null) {
            ...
            lg.setColorAt(0.5, new QColor(io.qt.core.Qt.GlobalColor.black));
    ...
    private void updateImage() {
        ...
        if (redCyan != 0) {
            QColor c = decideColor(redCyan, new QColor(io.qt.core.Qt.GlobalColor.cyan), new QColor(io.qt.core.Qt.GlobalColor.red));
            p.fillRect(0, 0, modified.width(), modified.height(), new QBrush(c));
        }
        if (greenMagenta != 0) {
            QColor c = decideColor(greenMagenta, new QColor(io.qt.core.Qt.GlobalColor.magenta), new QColor(io.qt.core.Qt.GlobalColor.green));
            p.fillRect(0, 0, modified.width(), modified.height(), new QBrush(c));
        }
        if (blueYellow != 0) {
            QColor c = decideColor(blueYellow, new QColor(io.qt.core.Qt.GlobalColor.yellow), new QColor(io.qt.core.Qt.GlobalColor.blue));
            p.fillRect(0, 0, modified.width(), modified.height(), new QBrush(c));
        }
        if (colorBalance != 0) {
            QColor c = decideColor(colorBalance, new QColor(io.qt.core.Qt.GlobalColor.white), new QColor(io.qt.core.Qt.GlobalColor.black));
            p.fillRect(0, 0, modified.width(), modified.height(), new QBrush(c));
        }

        if (invert) {
            p.setCompositionMode(QPainter.CompositionMode.CompositionMode_Difference);
            p.fillRect(modified.rect(), new QBrush(new QColor(io.qt.core.Qt.GlobalColor.white)));
        }
    ...
    private QImage createReflection(QImage source) {
        ...
        lg.setColorAt(0.6, new QColor(io.qt.core.Qt.GlobalColor.transparent));
