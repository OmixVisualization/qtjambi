...
public abstract class QMessageHandler {
    ...
    static void initialize() {
    	String config = System.getProperty("io.qt.exceptions-for-messages", "NONE");
        if (config != null) {
            config = config.trim().toUpperCase();
            boolean all = config.equals("") || config.equals("ALL") || config.equals("TRUE");
            boolean critical = config.contains("CRITICAL");
            boolean debug = config.contains("DEBUG");
            boolean fatal = config.contains("FATAL");
            boolean warning = config.contains("WARNING");

            if (all || critical || debug || fatal || warning) {
                QMessageHandler newMessageHandler = new MessageHandler(all, critical, debug, fatal, warning);
    ...
    private static void shutdown() {
        ...
        while(count < 999) {	// anti-infinite
        	...
        	java.util.logging.Logger.getLogger("io.qt.core").log(java.util.logging.Level.SEVERE, "QMessageHandler.shutdown(): removing handler " + h + "; remove your handler(s) before shutdown");
            removeMessageHandler(h);
            count++;
        }
    }
    
	private static class MessageHandler extends QMessageHandler{
		...
		private MessageHandler(boolean all, boolean critical, boolean debug, boolean fatal, boolean warning) {
			super();
			this.all = all;
	    ...
	    public void critical(String message) {
	        if (critical || all)
	        	...
	        	java.util.logging.Logger.getLogger("io.qt").severe(message);
	    }
	    ...
	    public void debug(String message) {
	        if (debug || all)
	        	...
	        	java.util.logging.Logger.getLogger("io.qt").finest(message);
	    }
	    ...
	    public void warning(String message) {
	        if (warning || all)
	        	...
	        	java.util.logging.Logger.getLogger("io.qt").warning(message);
	    }
	}
}
