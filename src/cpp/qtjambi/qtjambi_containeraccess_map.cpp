...
jobject AutoMapAccess::begin(JNIEnv * env, QtJambiNativeID ownerId, const void* container)
{
    QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
    return createIterator(env, ownerId, new void*(beginNode(map)));
}

jobject AutoMapAccess::end(JNIEnv * env, QtJambiNativeID ownerId, const void* container)
{
    QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
    return createIterator(env, ownerId, new void*(endNode(map)));
}

jobject AutoMapAccess::lowerBound(JNIEnv * env, QtJambiNativeID ownerId, const void* container, jobject key)
{
    QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
    QMapDataBase * d = *map;
    jvalue jv;
...
jobject AutoMapAccess::upperBound(JNIEnv * env, QtJambiNativeID ownerId, const void* container, jobject key)
{
    QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
    QMapDataBase * d = *map;
    jvalue jv;
...
bool AutoMapAccess::keyLessThan(JNIEnv * env, jobject key1, jobject key2)
{
    ...
    if(m_keyExternalToInternalConverter(env, &scope, jv, akey1, jValueType::l)){
        ...
        if(m_keyExternalToInternalConverter(env, &scope, jv, akey2, jValueType::l)){
            return is_less(m_keyMetaType, akey1, akey2);
        }
    }
    return false;
}
...
jobject AutoMapAccess::find(JNIEnv * env, QtJambiNativeID ownerId, const void* container, jobject key)
{
    QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
    QMapDataBase* d = *map;
    jvalue jv;
...
jobject AutoMapAccess::key(JNIEnv *env, const void* container, jobject value, jobject defaultKey)
{
    ...
    if(m_valueExternalToInternalConverter(env, &scope, jv, avalue, jValueType::l)){
        ...
        while (firstNode != lastNode) {
            if(is_equals(m_valueMetaType, reinterpret_cast<char*>(firstNode)+m_offset2, avalue)){
...
bool AutoMapAccess::equal(const void* a, const void* b){
    ...
    while (firstNode != lastNode && firstNode2 != lastNode2) {
        ...
        if(!is_equals(m_keyMetaType, key, key2))
            return false;
        void* value = reinterpret_cast<char*>(firstNode)+m_offset2;
        void* value2 = reinterpret_cast<char*>(firstNode2)+m_offset2;
        if(!is_equals(m_valueMetaType, value, value2))
            return false;
        firstNode = firstNode->nextNode();
...
jboolean AutoMapAccess::equal(JNIEnv *env, const void* container, jobject other)
{
    if ((*getIsBiContainerFunction())(env, other, keyMetaType(), valueMetaType())) {
        ...
        QMapDataBase *const* map = reinterpret_cast<QMapDataBase *const*>(container);
        QMapDataBase* d = *map;
        if(d->size!=qtjambi_map_size(env, other))
            return false;
        ...
        Node *firstNode(beginNode(map));
        Node *lastNode(endNode(map));
        while(firstNode != lastNode && qtjambi_iterator_has_next(env, iterator)){
            jobject entry = qtjambi_iterator_next(env, iterator);
            _value.l = qtjambi_map$entry_key(env, entry);
            _qvaluePtr = nullptr;
            if(!m_keyExternalToInternalConverter(env, &scope, _value, _qvaluePtr, jValueType::l))
                return false;
            void* key = reinterpret_cast<char*>(firstNode)+m_offset1;
            if(!is_equals(m_keyMetaType, key, _qvaluePtr))
                return false;
            _value.l = qtjambi_map$entry_value(env, entry);
            _qvaluePtr = nullptr;
        ...
        return firstNode == lastNode && !qtjambi_iterator_has_next(env, iterator);
    }
}
...
int AutoMapAccess::registerContainer(const QByteArray& typeName)
{
    ...
    if(newMetaType==QMetaType::UnknownType){
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
        {
            ...
            qtjambi_register_converter(new ConverterFunction(access, newMetaType, m_keyMetaType, m_valueMetaType, m_offset1, m_offset2), newMetaType, to);
        }
            ...
            struct DebugStreamFunction : QtPrivate::AbstractDebugStreamFunction{
                static void stream(const AbstractDebugStreamFunction *_f, QDebug& d, const void *ptr){
                    ...
                    while (firstNode != lastNode) {
                        ...
                        d << "(";
                        debug_stream(d, f->access->m_keyMetaType.id(), key);
                        d << ",";
                        debug_stream(d, f->access->m_valueMetaType.id(), value);
                        d << ")";
                        firstNode = firstNode->nextNode();
                    }
                    d << ")";
                                               ...
                                               qtjambi_function_pointer<16,void(QDataStream &, void *)>(
                                                                               [access](QDataStream &s, void * p){
                                                                                    ...
                                                                                    for(quint32 i=0; i<size; ++i){
                                                                                        ...
                                                                                        while (n) {
                                                                                            y = n;
                                                                                            if (!is_less(access->m_keyMetaType, reinterpret_cast<const char*>(n) + access->m_offset1, key)) {
                                                                                                left = true;
                                                                                                n = n->left;
                                                                                            } else {
                                                                                                left = false;
...
bool AutoMapAccess::qMapLessThanKey(const void* akey, const Node& right){
    return is_less(m_keyMetaType, akey, reinterpret_cast<const char*>(&right) + m_offset1);
}

bool AutoMapAccess::qMapLessThanKey(const Node& left, const void* akey){
    return is_less(m_keyMetaType, reinterpret_cast<const char*>(&left) + m_offset1, akey);
}
...
jobject AutoMapAccess::createIterator(JNIEnv * env, QtJambiNativeID ownerId, void* iteratorPtr)
{
    size_t offset1 = m_offset1;
    size_t offset2 = m_offset2;
    AutoBiIteratorAccess* containerAccess = new AutoBiIteratorAccess(m_valueInternalToExternalConverter,
                                                                     [](void*ptr){
