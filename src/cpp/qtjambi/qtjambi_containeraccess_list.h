...
public:
    ...
    jobject end(JNIEnv * env, QtJambiNativeID ownerId, const void* container) override {
        AbstractIteratorAccess* access = new IteratorAccess<QList, _align, _size, _isStatic>(
    ...
    jobject mid(JNIEnv * env, const void* container, jint index1, jint index2) override {
        ...
        jobject result = nullptr;
        {
            ...
            result = qtjambi_from_QList(env, mid, clone());
        }
...
struct ContainerAccessFac<QList,align,size,isStatic>{
    static constexpr ContainerAccessFactory factory(){
        return ContainerAccessFactory(GenericListAccess<align,size,isStatic>::newInstance);
    }
};

template<size_t align, size_t size, bool isStatic>
struct ContainerAccessFac<QQueue,align,size,isStatic> : ContainerAccessFac<QList,align,size,isStatic>{
};

#if QT_VERSION >= QT_VERSION_CHECK(6,0,0)
template<size_t align, size_t size, bool isStatic>
struct ContainerAccessFac<QStack,align,size,isStatic> : ContainerAccessFac<QList,align,size,isStatic>{
};
#endif
}
