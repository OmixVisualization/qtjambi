...
int AutoSetAccess::registerContainer(const QByteArray& typeName)
{
    ...
    if(newMetaType==QMetaType::UnknownType){
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
        {
            ...
            qtjambi_register_converter(new ConverterFunction(m_hashAccess.m_keyMetaType, m_hashAccess.m_offset1), newMetaType, to);
        }
        if(QMetaType::hasRegisteredComparators(m_hashAccess.m_keyMetaType.id())
            || qtjambi_registered_comparator(m_hashAccess.m_keyMetaType.id())
            || m_hashAccess.m_keyMetaType.id()<QMetaType::User
            || qtjambi_is_pointer_type(m_hashAccess.m_keyMetaType)){
        ...
        if(QMetaType::hasRegisteredDebugStreamOperator(m_hashAccess.m_keyMetaType.id())
                || qtjambi_registered_debugstream_operator(m_hashAccess.m_keyMetaType.id())
                || m_hashAccess.m_keyMetaType.id()<QMetaType::User
                || qtjambi_is_pointer_type(m_hashAccess.m_keyMetaType)){
...
jobject AutoSetAccess::begin(JNIEnv * env, QtJambiNativeID ownerId, const void* container)
{
    return m_hashAccess.begin(env, ownerId, container);
}

jobject AutoSetAccess::end(JNIEnv * env, QtJambiNativeID ownerId, const void* container)
{
    return m_hashAccess.end(env, ownerId, container);
}
...
void AutoSetAccess::intersect(JNIEnv * env, void* container, jobject other)
{
    ...
    if (qtjambi_is_QSet(env, other, elementMetaType())) {
        ...
        while(qtjambi_iterator_has_next(env, iterator)){
            m_hashAccess.insert(env, ptr, qtjambi_iterator_next(env, iterator), nullptr);
        }
    }
        ...
        if (*found2 == e2){
            ...
            if(*found != e){
                ...
                do {
                    ...
                    deleteNext = (next != e && is_equals(m_hashAccess.m_keyMetaType, key, reinterpret_cast<char*>(next)+m_hashAccess.m_offset1));
...
{
    if (qtjambi_is_QSet(env, other, elementMetaType())) {
        void* ptr = qtjambi_to_object(env, other);
        QHashData ** map = reinterpret_cast<QHashData **>(container);
        m_hashAccess.detach(map);
        QHashData*& d = *map;
        QHashData *const* map2 = reinterpret_cast<QHashData *const*>(ptr);
        ...
        jobject iterator = qtjambi_collection_iterator(env, other);
        while(qtjambi_iterator_has_next(env, iterator)){
            m_hashAccess.insert(env, container, qtjambi_iterator_next(env, iterator), nullptr);
        }
    }
}
