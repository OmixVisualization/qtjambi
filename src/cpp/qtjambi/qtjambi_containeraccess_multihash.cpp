...
jobject AutoMultiHashAccess::begin(JNIEnv *env, QtJambiNativeID ownerId, const void* container) {return AutoHashAccess::begin(env, ownerId, container);}
...
jobject AutoMultiHashAccess::end(JNIEnv *env, QtJambiNativeID ownerId, const void* container) {return AutoHashAccess::end(env, ownerId, container);}
jobject AutoMultiHashAccess::find(JNIEnv *env, QtJambiNativeID ownerId, const void* container, jobject key) {return AutoHashAccess::find(env, ownerId, container, key);}
...
void AutoMultiHashAccess::unite(JNIEnv *env, void* container, jobject other)
{
    ...
    if (qtjambi_is_QMultiHash(env, other, keyMetaType(), valueMetaType())
            || qtjambi_is_QHash(env, other, keyMetaType(), valueMetaType())) {
    ...
    if(m_keyExternalToInternalConverter(env, &scope, jv, akey, jValueType::l)){
        ...
        if(Node *n = *findNode(map, akey)){
            ...
            if(m_valueExternalToInternalConverter(env, &scope, jv, avalue, jValueType::l)){
                Node *lastNode(reinterpret_cast<Node*>(d));
                do{
                    if(is_equals(m_valueMetaType, avalue, reinterpret_cast<char*>(n)+m_offset2))
                        return true;
                    n = QHashData::nextNode(n);
...
jobject AutoMultiHashAccess::find(JNIEnv *env, QtJambiNativeID ownerId, const void* container, jobject key, jobject value)
{
    QHashData *const* map = reinterpret_cast<QHashData *const*>(container);
    QHashData* d = *map;
    jvalue jv;
