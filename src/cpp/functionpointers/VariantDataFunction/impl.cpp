...
typedef QtMetaTypePrivate::VariantData(*Call)(QFunctionPointer fn, int, uint);
...
struct FunctionInitializer{
    ...
    static QtMetaTypePrivate::VariantData f(int id, uint flg) {
        if(!call[N])reinterpret_cast<QFunctionPointer>(call[0])();
        return call[N](reinterpret_cast<QFunctionPointer>(&f), id, flg);
    }
};
