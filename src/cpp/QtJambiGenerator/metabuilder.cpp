...
inline void hide_functions(const MetaFunctionList &l) {
    for(MetaFunction *f : l) {
        if(!f->declaringClass()->isInterface()
            ...
            mod.originalSignature = f->originalSignature();
...
void MetaBuilder::checkFunctionModifications() {
    ...
    for(TypeEntry *entry : entries) {
        if (!entry || (entry->codeGeneration() & ~TypeEntry::InheritedByTypeSystem) == TypeEntry::GenerateNothing)
            continue;
        if (entry->isTypeSystem()){
            ...
            for(const FunctionModification& modification : functionModifications) {
                ...
                if(!name.isEmpty()){
                    ...
                    if(modification.signature.indexOf('<')>0){
                        name = name.mid(0, modification.signature.indexOf('<'));
                    }
                    ...
                    for(MetaFunction *function : clazz->functions()) {
                        if ((function->minimalSignature() == modification.signature || function->originalSignature() == modification.originalSignature)
                                && function->implementingClass() == clazz) {
                            found = true;
                            break;
                        }
                    ...
                    if(!found && (clazz->typeEntry()->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)!=TypeEntry::GenerateAll){
                        for(MetaFunction *function : clazz->invalidFunctions()) {
                            if (function->originalSignature() == modification.originalSignature) {
                                found = true;
                                break;
                            }
                        }
...
//              meta_enum->typeEntry()->setQualifier(globalName);
    ...
    for(const TypeAliasModelItem& tai : m_dom->typeAliases()){
        if(tai->type().qualifiedName()==QStringList{"std","function"}
                || tai->type().qualifiedName()==QStringList{"std","move_only_function"}
    ...
    QList<PendingDelegate> pendingDelegates;
    for(MetaClass *cls : qAsConst(m_meta_classes)) {
        if (!cls->isInterface() && !cls->isNamespace()) {
            setupInheritance(cls, pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
        }else if(cls->isInterface()){
            setupInheritance(cls->extractInterfaceImpl(), pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
        }
    }
    ...
    for(const PendingDelegate& pendingDelegate : qAsConst(pendingDelegates)){
        if (MetaClass *base_class = m_meta_classes.findClass(pendingDelegate.base_class_type)) {
            for(MetaFunction* function : base_class->functions()){
                const FunctionModificationList mods = function->modifications(pendingDelegate.meta_class);
                for(const FunctionModification& mod : mods){
                    if(mod.modifiers & TS::Modification::PullDown && mod.association==pendingDelegate.base_class_type->name()){
                        function = function->copy();
                        if(function->type() && function->type()->typeEntry()==pendingDelegate.base_class_type){
                            function->type()->setTypeEntry(pendingDelegate.meta_class->typeEntry());
                        }
                        if(pendingDelegate.is_protected){
                            *function -= MetaAttributes::Public;
                            *function += MetaAttributes::Protected;
                        }
                        function->setImplementingClass(pendingDelegate.meta_class);
                        function->setDeclaringClass(pendingDelegate.meta_class);
                        pendingDelegate.meta_class->addFunction(function);
                        break;
                    }
                }
            }
    ...
    for(const TypeEntry *entry : entries) {
        ...
        if (entry->isEnum()) {
            ...
            if((entry->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)==TypeEntry::GenerateAll){
                if (!cls) {
    ...
    for(MetaClass *cls : qAsConst(m_meta_classes)) {
        ...
        if(cls->hasCloneOperator() && !cls->typeEntry()->isContainer() && !cls->isInterface()){
            ...
            if(!hasCopyConstructor){
                ...
                for(MetaFunction *f : functions) {
                    if (!f->isInvalid()
                        && !f->isEmptyFunction()
                        && !f->isFake()
                        && f->implementingClass()==cls
                        && f->actualMinimumArgumentCount()==1
                        && f->arguments()[0]->type()->typeEntry()==cls->typeEntry()
                        && (
                            (f->arguments()[0]->type()->isConstant() && f->arguments()[0]->type()->getReferenceType()==MetaType::Reference)
                            || f->arguments()[0]->type()->getReferenceType()==MetaType::NoReference)) {
            ...
            if(!hasCopyConstructor){
                ...
                *copyConstructor += MetaAttributes::Public;
                *copyConstructor += MetaAttributes::Native;
                *copyConstructor += MetaAttributes::Final;
                ...
                arg->setType(type);
                copyConstructor->addArgument(arg);
                copyConstructor->setDeclaringClass(cls);
                copyConstructor->setImplementingClass(cls);
                copyConstructor->setOriginalSignature(copyConstructor->minimalSignature());
                copyConstructor->setOriginalAttributes(copyConstructor->attributes());
                cls->addFunction(copyConstructor);
                //ReportHandler::warning(QStringLiteral("Added implicit copy constructor for type %1").arg(cls->qualifiedCppName()));
            }
        }
    }
            ...
            if(meta_class->isNamespace() || meta_class->isFake()){
                ...
                for(MetaFunction * meta_function : meta_class->functions()){
                    ...
                    if(meta_function->functionType()==MetaFunction::GlobalScopeFunction){
                        if(meta_function->functionTemplate().first){
                            if(ns){
                                functions = ns->getFunctions(meta_function->functionTemplate().first->originalName());
                            }
                            if(functions.isEmpty() && globalNamespace){
                                functions = globalNamespace->getFunctions(meta_function->functionTemplate().first->originalName());
                            }
                        }else{
                            if(ns){
                        ...
                        for(const DocFunction* function : qAsConst(functions)){
                            if(meta_function->isConstant()==function->isConst()
                                ...
                                if(ok){
                                    ...
                                    ok = true;
                                    for(int i=0; i<meta_function->functionTemplate().first->arguments().size(); ++i){
                                        MetaArgument *argument = meta_function->functionTemplate().first->arguments()[i];
                                        QString arg = function->parameters()[i];
                                        if(arg.endsWith("...")){
                                            arg.chop(3);
                                            if(!meta_function->functionTemplate().first->isVariadics()){
                                                ok = false;
                                                break;
                                            }
                                        }
                ...
                if(cls){
                    ...
                    for(MetaFunction * meta_function : meta_class->functions()){
                        ...
                        if(meta_function->declaringClass() && meta_function->declaringClass()!=meta_class){
...
void analyzeFunctional(MetaFunctional* meta_functional, const QList<MetaArgument*>& actualArguments){
    ...
    if((meta_functional->typeEntry()->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)==TypeEntry::GenerateNothing && !meta_functional->javaFunctionalInterface().isEmpty()){
    ...
    if(!_using.isEmpty()){
        ...
        if(ftype){
            ...
            if(usingType.qualifiedName()==QStringList{"std", "function"}
                    || (is_mof = usingType.qualifiedName()==QStringList{"std", "move_only_function"})
                    || (is_qxp = usingType.qualifiedName()==QStringList{"qxp", "function_ref"})
                    || usingType.isFunctionPointer()){
                QString normalizedSignature = usingType.isFunctionPointer() ? "" : (is_qxp ? "qxp::function_ref<" : (is_mof ? "std::move_only_function<" : "std::function<"));
                QString newUsing = usingType.isFunctionPointer() ? "QtJambiAPI::FunctionType<" : (is_qxp ? "qxp::function_ref<" : (is_mof ? "std::move_only_function<" : "std::function<"));
...
TypeInfo MetaBuilder::convertInfo(MetaClass *cls, const TypeParser::Info& ti){
    ...
    if(!ti.arrays.isEmpty()){
        TypeInfo aTypeInfo;
        aTypeInfo.setArray(true);
        aTypeInfo.setArrayType(info);
        aTypeInfo.setArrayElements(ti.arrays);
        info = aTypeInfo;
    }else{
...
MetaClass *MetaBuilder::traverseClass(ClassModelItem class_item, QList<PendingClass>& pendingClasses, QList<PendingFunctional>& pendingFunctionals) {
    ...
    if (reason == NoReason) {
        ...
        } else if ((type->codeGeneration() & ~TypeEntry::InheritedByTypeSystem) == TypeEntry::GenerateNothing) {
            reason = GenerationDisabled;
        }
    }
    ...
    *meta_class += MetaAttributes::Public;
    ...
    if(class_item.data()->classType()==CodeModel::Union){
        *meta_class += MetaAttributes::Final;
        *meta_class += MetaAttributes::IsUnion;
    }
...
MetaField *MetaBuilder::traverseField(VariableModelItem field) {
    ...
    if (!meta_type && m_current_class) {
        ...
        if(!mod.modified_type.isEmpty()){
            ...
            meta_type->setTypeEntry(new ObjectTypeEntry("decltype(std::declval<" + m_current_class->qualifiedCppName() + ">()." + field_name + ")"));
            ok = true;
        }
    }
...
void MetaBuilder::traverseFields(ScopeModelItem scope_item) {
    for(const VariableModelItem& field : scope_item->variables()) {
        ...
        if (meta_field) {
            ...
            if(m_current_class){
                MetaClass* targetClass = m_current_class;
                if(meta_field->isStatic()){
                    while(targetClass->baseClass() && targetClass->typeEntry()->getPushUpStatics()){
                        targetClass = targetClass->baseClass();
                    }
                    if(targetClass != m_current_class){
                        if(meta_field->type()->typeEntry()==m_current_class->typeEntry()){
                ...
                if(meta_field->type()->isSmartPointer()){
                    if(field->type().toString().startsWith("QScopedPointer<")
                        ...
                        if(!targetClass->typeEntry()->hasPrivateCopyConstructor()
                            && !targetClass->typeEntry()->hasProtectedCopyConstructor()
                            && !targetClass->typeEntry()->hasPublicCopyConstructor()){
                            applyOnType(targetClass->typeEntry(), [](ComplexTypeEntry* c){c->setHasPrivateCopyConstructor();});
                        }
                        if(!targetClass->typeEntry()->hasPrivateMoveConstructor()
                            && !targetClass->typeEntry()->hasProtectedMoveConstructor()
                            && !targetClass->typeEntry()->hasPublicMoveConstructor()){
                ...
                if(targetClass->typeEntry()->designatedInterface()){
                    targetClass->extractInterface()->addField(meta_field);
                    if(meta_field->isPublic()){
                        applyOnType(targetClass->typeEntry(), [](ComplexTypeEntry* c){c->setHasFields();});
                    }else{
                        applyOnType(targetClass->typeEntry(), [](ComplexTypeEntry* c){c->setHasNonPublicFields();});
                    }
                }else{
                    targetClass->addField(meta_field);
                    if(meta_field->isPublic()){
                        applyOnType(targetClass->typeEntry(), [](ComplexTypeEntry* c){c->setHasFields();});
                    }else{
                        applyOnType(targetClass->typeEntry(), [](ComplexTypeEntry* c){c->setHasNonPublicFields();});
...
void MetaBuilder::traverseFunctions(ScopeModelItem scope_item) {
    ...
    for(const FunctionModelItem& function_item : qAsConst(functions << templateInstantiations)) {
        ...
        if(function_item->isTemplate()){
            ...
            for(TemplateParameterModelItem t : function_item->templateParameters()){
                if(t->isVaradic()){
                    ...
                    if(t->name().isEmpty()){
                        if(t->defaultValue().isEmpty())
                            tparams << t->parameterTypeInfo().toString();
                        else
                            tparams << t->defaultValue();
                    }else{
                        tparams << t->name();
                    }
                }
        ...
        QStringList function_scope = function_item->scope();
        if(m_current_class){
            if(function_scope.join("::")==m_current_class->qualifiedCppName()){
        ...
        if (meta_function) {
            ...
            TS::Language removalMod = TS::NoLanguage;
                    ...
                    if(!ts->functionModifications().isEmpty()){
                        ...
                        for(const FunctionModification& mod : qAsConst(list)){
                            if(mod.removal & TS::TargetLangCode){
                                removalMod = mod.removal;
                            }
                            if(mod.modifiers & TS::Modification::IsTextStreamFunction){
                            ...
                            if(!mod.targetType.isEmpty()){
                                ...
                                if(!targetClass){
                                    ...
                                    *targetClass += MetaAttributes::Public;
                                    ...
                                    MetaFunctionList otherFunctions = targetClass->queryFunctionsByName(meta_function->name());
                                    if(targetClass->isInterface()){
                                        if(MetaClass *ifaceImpl = targetClass->extractInterfaceImpl())
                                            otherFunctions << ifaceImpl->queryFunctionsByName(meta_function->name());
                                    }
                                    for(MetaFunction *other : qAsConst(otherFunctions)){
                                        if(!other->isRemovedFrom(targetClass, TS::TargetLangCode)
            ...
            if(!targetClass && removalMod!=TS::NoLanguage){
                if(isTextStreamFunction){
                    m_textStreamFunctions << meta_function;
                }else{
                    delete meta_function;
                    meta_function = nullptr;
                }
            ...
            if(targetClass){
                ...
                if(meta_function->isStatic() && !meta_function->isConstructor() && !meta_function->isDestructor() && !meta_function->isEmptyFunction() && !meta_function->isInvalid() && !meta_function->isInGlobalScope()){
                    ...
                    if(previousClass != targetClass){
                        if(meta_function->type() && meta_function->type()->typeEntry()==previousClass->typeEntry()){
                            meta_function->type()->setTypeEntry(targetClass->typeEntry());
                        }
                        for(MetaArgument* arg : meta_function->arguments()){
                            if(arg->type() && arg->type()->typeEntry()==previousClass->typeEntry()){
                                arg->type()->setTypeEntry(targetClass->typeEntry());
                            }
                        }
                    }
                }
                ...
                if(!skip && (!targetClass
                    ...
                    if(meta_function->operatorType()!=OperatorType::None){
                        ...
                        case OperatorType::Rem:
                            if(arguments.size()>=1 && (arguments[0]->type()->getReferenceType()!=MetaType::RReference || arguments.size()>1)){
                            ...
                            }else if(!targetClassGlobalSet || (arguments.size()>=1 && arguments[0]->type()->getReferenceType()==MetaType::RReference)){
                                targetClass = nullptr;
                                skip = true;
                            }
                            break;
                                    ...
                                    if(meta_function->operatorType()==OperatorType::ShiftLeft && arguments[0]->type()->typeEntry()->qualifiedCppName()=="QDebug"){
                                        ...
                                        if (cls && !cls->typeEntry()->getNoInstance()) {
                                            ...
                                            *meta_function -= MetaAttributes::Static;
                                            ...
                                            FunctionModification mod;
                                            mod.originalSignature = oldMinimalSignature;
                                            mod.signature = meta_function->minimalSignature();
                                            mod.modifiers |= Modification::NonFinal;
                                            cls->typeEntry()->addFunctionModification(mod);
                                            targetClass = cls;
                                            const MetaFunctionList otherFunctions = targetClass->queryFunctionsByName(meta_function->name());
                                            for(MetaFunction *other : otherFunctions){
                                                if(!other->isRemovedFrom(cls, TS::TargetLangCode)
                                                        && other->arguments().size()==arguments.size()){
                                                    bool skip = true;
                                            ...
                                            auto typeEntry = arguments.at(1)->type()->typeEntry();
                                            if(!(typeEntry->codeGeneration() & TypeEntry::GenerateCpp)
                                                    || typeEntry->isPrimitive()
                                                    || typeEntry->isEnum()
                                                    || typeEntry->isFlags()
...
//                                    skip = true;
                        ...
                        default:
                            if(arguments.isEmpty() || (
                                ...
                                }else if(meta_function->type()){
                                    auto typeEntry = meta_function->type()->typeEntry();
                                    if(!(typeEntry->codeGeneration() & TypeEntry::GenerateCpp)
                                            || typeEntry->isPrimitive()
                                            || typeEntry->isEnum()
                                            || typeEntry->isFlags()
                            ...
                            }else if(arguments.size()>=1 && arguments[0]->type()->getReferenceType()==MetaType::RReference){
                                targetClass = nullptr;
                                skip = true;
                            }
                            break;
                        ...
                        if (!arguments.isEmpty()){
                            ...
                            if(!(typeEntry->codeGeneration() & TypeEntry::GenerateCpp)
                                ...
                                if(cls->typeEntry()->hasHash()){
                                    skip = true;
                                }else if (arguments.size() == 1 || hasDefaultArgs || isWorkaround){
                                    addInclude(cls->typeEntry(), function_item->fileName(), true);
                        ...
                        if(!skip && !meta_function->templateParameters().isEmpty())
                            skip = true;
                        if(!skip){
                            m_pendingHashFunctions << QPair<QPair<QString,FunctionModelItem>,MetaFunction*>{{_function_name, function_item}, meta_function};
                            skip = true;
                        }
                        ...
                        if(arguments.size()==2
                            ...
                            if(!skip){
                                m_pendingSwapFunctions << QPair<QPair<QString,FunctionModelItem>,MetaFunction*>{{_function_name, function_item}, meta_function};
                                skip = true;
                            }
                        }
                ...
                if(!skip){
                    if(targetClass){
                        ...
                        if(!meta_function->originalSignature().isEmpty()
                                && ((function_item->isInline() && !meta_function->isConstructor())
                                    || (!function_item->isInline() && (function_item->hasBody() || function_item->isDeclDefault()))
                                    || (function_item->isFriend()))){
                            for(MetaFunction *f : targetClass->queryFunctionsByOriginalName(meta_function->originalName())) {
...
void MetaBuilder::checkHashAndSwapFunctions(){
    bool skip = false;
    for(const QPair<QPair<QString,FunctionModelItem>,MetaFunction*>& pair : qAsConst(m_pendingHashFunctions)){
    ...
    for(const QPair<QPair<QString,FunctionModelItem>,MetaFunction*>& pair : qAsConst(m_pendingSwapFunctions)){
        const QString& _function_name = pair.first.first;
        const FunctionModelItem& function_item = pair.first.second;
        MetaFunction* meta_function = pair.second;
        skip = false;
        ...
        if (MetaClass *cls = argumentToClass(arguments.at(0)->type())) {
            ...
            if(!skip){
                ...
                *meta_function += MetaAttributes::Public;
                *meta_function -= MetaAttributes::Static;
                QString oldMinimalSignature = meta_function->minimalSignature();
                MetaArgumentList arguments = meta_function->arguments();
                arguments.removeFirst();
                meta_function->setArguments(arguments);
                ...
                if(!oldMinimalSignature.isEmpty() && oldMinimalSignature!=meta_function->minimalSignature()){
                    for(const FunctionModification& mod : cls->typeEntry()->functionModifications()){
                        if (mod.signature == oldMinimalSignature) {
                            const_cast<QString&>(mod.signature) = meta_function->minimalSignature();
                        }
                    }
...
MetaType* MetaBuilder::exchangeTemplateTypes(const MetaType* type, bool isReturn, const QMap<QString,QPair<MetaType*,QString>>& templateTypes){
    if(!type)
        return nullptr;
    if(type->typeEntry()->isTemplateArgument()){
        QPair<MetaType*,QString> instantiation = templateTypes[type->typeEntry()->name()];
        if(instantiation.first){
            MetaType*rtype = type->copy();
            rtype->setConstant(instantiation.first->isConstant() || type->isConstant());
            rtype->setIndirections(QList<bool>() << instantiation.first->indirections() << type->indirections());
            ...
            rtype->setTypeEntry(instantiation.first->typeEntry());
        ...
        MetaType *arrayElementType = exchangeTemplateTypes(type->arrayElementType(), false, templateTypes);
        QList<QPair<int,QString>> arrayElementCount = type->arrayElementCounts();
        bool arrayElementCountChanged = false;
        bool setCount;
        for(QPair<int,QString>& pair : arrayElementCount){
            setCount = false;
            if(!pair.second.isEmpty()){
                QPair<MetaType*,QString> instantiation = templateTypes[pair.second];
                if(!instantiation.second.isEmpty()){
                    pair.first = instantiation.second.toInt(&setCount);
                    if(setCount){
                        arrayElementCountChanged = true;
                        pair.second.clear();
                        instantiation.second.clear();
                    }
                }
            }
        }
        ...
        if(type->instantiations()!=instantiations || arrayElementType!=type->arrayElementType() || arrayElementCountChanged){
            MetaType*rtype = type->copy();
            if(arrayElementType!=type->arrayElementType()){
                delete rtype->arrayElementType();
                rtype->setArrayElementType(arrayElementType);
            }
            rtype->setInstantiations(instantiations);
            if(arrayElementCountChanged){
                rtype->setArrayElementCounts(arrayElementCount);
            }
            return rtype;
        }
    }
    ...
    for(MetaFunction *func : meta_class->functions()) {
        ...
        if(!templateParameters.isEmpty() || (func->type() && func->type()->typeUsagePattern()==MetaType::AutoPattern)){
            ...
            for(const FunctionModification& mod : qAsConst(mods)) {
                for(const TemplateInstantiation& ti : mod.template_instantiations){
                    templateInstantiations.append({&ti, &mod});
                }
            }
                        ...
                        if(tparam->instantiation().isEmpty() && !tparam->instantiationType()){
                            if(tparam->type() && tparam->type()->typeUsagePattern()==MetaType::TemplateArgumentPattern){
                                if(template_instantiation.arguments[k].name.isEmpty()){
                                    ...
                                    if(arg=="void"){
                                        if(TypeEntry * te = TypeDatabase::instance()->findType("void")){
                                            MetaType *ttype = new MetaType();
...
//                            untreatedTemplateParameters.removeOne(tparam);
                    ...
                    for(int k=template_instantiation.arguments.size(); k<func2->templateParameters().size(); k++){
                        MetaTemplateParameter* tparam = func2->templateParameters()[k];
                        /*if(tparam->type() && tparam->type()->typeUsagePattern()==MetaType::TemplateArgumentPattern){
                            if(!tparam->defaultType().isEmpty())
                                untreatedTemplateParameters.removeOne(tparam);
                        }else{
                        }*/
                        if(!tparam->defaultType().isEmpty() || !tparam->instantiation().isEmpty())
                            untreatedTemplateParameters.removeOne(tparam);
                    }
                    ...
                    if(func2->type()){
                        if(func2->type()->typeUsagePattern()==MetaType::AutoPattern){
                            ...
                            if(rtype!=func2->type()){
                                ...
                                if(rtype->typeUsagePattern()==MetaType::VoidPattern){
                                    func2->setType(rtype);
                                }else{
                                    func2->setType(rtype);
                                }
                    ...
                    if(add){
                        {
                            ...
                            if(mod.proxyCall.isEmpty())
                                mod.proxyCall = template_instantiation.proxyCall;
                            mod.argument_mods << template_instantiation.argument_mods;
                            mod.delegates << template_instantiation.delegates;
...
//                            meta_class->typeEntry()->addFunctionModification(mod);
                ...
                for(int k=0; k<func2->templateParameters().size(); k++){
                    ...
                    if(tparam->type() && tparam->type()->typeUsagePattern()==MetaType::TemplateArgumentPattern){
                        ...
                        if(ok && ttype){
                            templateTypes[tparam->name()] = {ttype,{}};
                            tparam->setInstantiation(true, tparam->defaultType(), ttype);
                        }
                    }
                }
                if(func2->type()){
                ...
                functions << func2.release();
            }else if(func->hasUnresolvedTemplateTypes()){
                ...
                if(func->isRemovedFromAllLanguages(meta_class)){
                    ...
                    bool resolved = true;
                    for(MetaTemplateParameter* tparam : qAsConst(templateParameters)){
                        if(tparam->defaultType().isEmpty() && tparam->instantiation().isEmpty() && !tparam->instantiationType()){
                            resolved = false;
                            break;
                        }
                    }
...
//     printf("fix functions for %s\n", qPrintable(name()));
    ...
    if (super_class){
        ...
        if(super_class->typeEntry()->codeGeneration() == TypeEntry::GenerateForSubclass){
            for(MetaField* field : super_class->fields()){
                if(!cls->findField(field->name())){
                    field = field->copy();
                    field->setEnclosingClass(cls);
                    cls->addField(field);
                }
            }
        }
...
void MetaBuilder::setupInheritance(MetaClass *meta_class, QList<PendingHiddenBaseType>& pendingHiddenBaseTypes, QList<MetaClass *>& pendingConstructorUsages, QList<QPair<MetaClass*,MetaClass*>>& pendingPrivateSuperUsages, QList<PendingDelegate>& pendingDelegates) {
    ...
    if(meta_class->templateBaseClass() && meta_class->qualifiedCppName().endsWith(">")){
        setupInheritance(const_cast<MetaClass *>(meta_class->templateBaseClass()), pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
    ...
    if (publicBaseClasses.size() == 1 && !publicBaseClasses.first().arguments().isEmpty()) {
        ...
        //for (auto i = scope.size(); i >= 0; --i)
        {
            ...
            if (templ) {
                setupInheritance(templ, pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
    ...
    for (int i = 0; i < publicBaseClasses.size(); ++i) {
        ...
        if(meta_class->typeEntry()->delegatedBaseClasses().contains(base_class_name)){
            ...
            delegateFunction->setOriginalAttributes(MetaAttributes::Final | MetaAttributes::Public);
            ...
            *delegateFunction += MetaAttributes::Native;
            ...
            pendingDelegates << PendingDelegate{meta_class, base_class_entry};
            continue;
        }

        // true for primary base class
        if (!base_class_entry->designatedInterface() && !base_class_entry->isInterface()) {
            if (primaries > 0) {
    ...
    for (int i = 0; i < protectedBaseClasses.size(); ++i) {
        ...
        if (!base_class_entry) {
            ...
            *delegateFunction += MetaAttributes::Native;
            ...
            delegateFunction->setImplementingClass(meta_class);
            meta_class->addFunction(delegateFunction);
            pendingDelegates << PendingDelegate{meta_class, base_class_entry, true};
        }
    }
    ...
    if (primary >= 0) {
        ...
        if(base_class->isInterface()){
            setupInheritance(const_cast<MetaClass *>(base_class)->extractInterfaceImpl(), pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
            if(!meta_class->interfaces().contains(base_class))
                meta_class->addInterface(base_class);
        }else{
            setupInheritance(base_class, pendingHiddenBaseTypes, pendingConstructorUsages, pendingPrivateSuperUsages, pendingDelegates);
...
MetaFunction *MetaBuilder::traverseFunction(FunctionModelItem function_item, const QString& _function_name, const QStringList& tparams) {
    ...
    if (!remove && !m_current_class) {
        ...
        for(const TypeSystemTypeEntry* ts : typeSystems){
            ...
            for(const FunctionModification& mod : qAsConst(list)){
                if(mod.isRemoveModifier() && !mod.isTextStreamFunction()){
                    remove = true;
                    break;
                }
            }
    ...
    *meta_function += MetaAttributes::Native;
    ...
    for(const TemplateParameterModelItem& p : function_item->templateParameters()){
        if(!p->ownerClass()){
            ...
            if(p->parameterTypeInfo().qualifiedName().isEmpty()){
    ...
    if (m_current_class && !m_current_class->isNamespace() && function_name.startsWith('~')) {
        ...
        bool ok = false;
        MetaType *type = nullptr;
        if(!function_type.qualifiedName().isEmpty() && metaTemplateParameters.contains(function_type.qualifiedName().join("::"))){
            MetaTemplateParameter* parameter = metaTemplateParameters[function_type.qualifiedName().join("::")];
        ...
        if(!ok){
            ...
            if(m_current_class){
                ...
                if(list.isEmpty() && _originalSignature!=signature)
            ...
            for(const FunctionModification& mod : qAsConst(list)){
                ...
                if(!ok && templateParameterCounter>0 && !mod.template_instantiations.isEmpty()){
                    for(const TemplateInstantiation& templateInstantiation : mod.template_instantiations){
                        for(const ArgumentModification& amod : templateInstantiation.argument_mods){
                            if(amod.index==0 && !amod.modified_type.isEmpty()){
                                TypeInfo _type = function_type;
                                _type.setVolatile(false);
                                _type.setConstant(false);
        ...
        if (!ok) {
            if(function_item->accessPolicy() != CodeModel::Private
                && m_current_class && (m_current_class->typeEntry()->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)==TypeEntry::GenerateAll
                ...
                if(function_item->functionType() == CodeModel::Signal
                    ...
                    ReportHandler::warning(QString("skipping signal '%1::%2', unmatched return type '%3'")
                                           .arg(class_name, _function_name, function_item->type().toString()));
                    ...
                    ReportHandler::warning(QString("skipping function '%1::%2<%3>', unmatched return type '%4'")
                                           .arg(class_name, _function_name, tparams.join(", "), function_item->type().toString()));
    ...
    QString originalSignature = function_item->name() + QChar(u'(');
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        ...
        if(!arg->type().isFunctionPointer()
                && arg->type().arrayElements().isEmpty()
                && metaTemplateParameters.contains(arg->type().qualifiedName().join("::"))){
            ...
            for(const FunctionModification& mod : qAsConst(list)){
                ...
                for(const ArgumentModification& amod : mod.argument_mods){
                    if((amod.index==i+1 && (!amod.modified_type.isEmpty() || amod.removed))
                            || (amod.useAsSlotType.testFlag(AsSlotType::Yes) && amod.utilArgParameter==i+1)){
            ...
            if(function_item->accessPolicy() != CodeModel::Private
                ...
                if(function_item->functionType() == CodeModel::Signal
                    ...
                    ReportHandler::warning(QString("skipping signal '%1::%2', "
                                           ...
                                           .arg(class_name, _function_name, arg->type().toString()));
                    ...
                    ReportHandler::warning(QString("skipping function '%1::%2<%3>', "
                                           ...
                                           .arg(class_name, _function_name, tparams.join(", "), arg->type().toString()));
...
//        }
    ...
    if(!originalSignature.contains("std::function<") && !originalSignature.contains("qxp::function_ref<") && !originalSignature.contains("(*)") && !originalSignature.contains("(&)")){
        originalSignature = QString::fromLatin1(QMetaObject::normalizedSignature(qPrintable(originalSignature)));
    }
    if(!meta_function->templateParameters().isEmpty()){
        QStringList args;
        for(MetaTemplateParameter* param : meta_function->templateParameters()){
            if(param->type() && param->type()->isTemplateArgument()){
                QString strg = param->type()->name();
                if(strg.isEmpty()){
        ...
        if(!args.isEmpty()){
            originalSignature = function_item->name()
                    + "<" + args.join(",") + ">"
                    + originalSignature.mid(function_item->name().length());
        }
    }
    ...
    bool isReturnChanged = false;
    if(m_current_class){
        FunctionModificationList mods = meta_function->modifications(m_current_class->typeEntry());
        for(const FunctionModification& mod : mods){
            if(mod.removal & TS::TargetLangCode){
                isRemoved = true;
            }
    ...
    for (int i = 0; i < meta_arguments.size(); ++i) {
        ...
        if (arg->defaultValue()) {
            //if(!meta_arg->type()->isTemplateArgument()){
                QString expr = arg->defaultValueExpression();
                if (!expr.isEmpty())
...
MetaType *MetaBuilder::translateType(TypeInfo typei,
    ...
    *ok = true;
    ...
    if(typei.isExpression()){
        MetaType *targ_type = new MetaType();
        TemplateArgumentEntry* tae = new TemplateArgumentEntry(typei.expression());
        targ_type->setTypeUsagePattern(MetaType::TemplateArgumentPattern);
        targ_type->setTypeEntry(tae);
        *ok = true;
        return targ_type;
    }
    ...
    if (typei.arrayElements().size() > 0) {
        ...
        if (!array_of_unspecified_size) {
            MetaType *elementType = translateType(typei.arrayType(), ok, contextString, true, resolveScope, prependScope);
            if (!elementType || !ok) {
                if(prependScope)
                    ReportHandler::warning("Something has happened when trying to resolve array type " + typei.toString() + " in " + contextString);
                return nullptr;
            }

            QList<QPair<int,QString>> arrayElementCount;
            for (const QString& s : typei.arrayElements()) {
                if(s.isEmpty()){
                    arrayElementCount << QPair<int,QString>(0, s);
                }else{
                    bool ok;
                    ...
                    if (!ok) {
                        if (m_current_function) {
                            for(MetaTemplateParameter* t : m_current_function->templateParameters()){
                                if (t->instantiation().isEmpty() && t->name() == s){
                                    isTemplate = true;
                                    ok = true;
                                    break;
                                }
                    ...
                    if(isTemplate)
                        ...
                        arrayElementCount << QPair<int,QString>(elems, {});
                }
            }
            MetaType *arrayType = new MetaType();
            arrayType->setArrayElementCounts(arrayElementCount);
            arrayType->setArrayElementType(elementType);
            arrayType->setReferenceType(MetaType::ReferenceType(typei.getReferenceType()));
            arrayType->setTypeEntry(new ArrayTypeEntry(typei.arrayType().toString(), elementType->typeEntry(), int(arrayElementCount.size())));
            decideUsagePattern(arrayType);

            //qDebug()<<"Returning element type:"<<elementType->fullName();
            return arrayType;
        }  else {
            TypeInfo _typei = typei.arrayType();
            QList<bool> indirections = _typei.indirections();
            for(int i=0; i<typei.arrayElements().size(); i++){
                indirections << false;
            }
            _typei.setIndirections(indirections);
            typei = _typei;
        }
    }
    ...
    if(type && (type->isObject() || type->isValue())
        ...
        && ((type->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)==TypeEntry::GenerateNothing
            || (type->codeGeneration() & ~TypeEntry::InheritedByTypeSystem)==TypeEntry::GenerateForSubclass)
    ...
    if (!type && m_current_function) {
        for(MetaTemplateParameter* t : m_current_function->templateParameters()){
            if (t->instantiation().isEmpty() && t->type() && t->type()->isTemplateArgument() && t->type()->typeEntry()->name() == qualified_name){
        ...
        for(TypeInfo info : typei.arguments()) {
            ...
            if (!_ok) {
                ...
                if(backup_type){
                    ...
                    const TypeEntry *type = m_database->findType("auto");
                    if(type){
                        targ_type = new MetaType();
                        targ_type->setTypeUsagePattern(MetaType::AutoPattern);
                        targ_type->setTypeEntry(type);
                    }else {
                        delete meta_type;
                        return nullptr;
            ...
            if(!targ_type && info.toString()=="void"){
                ...
                if(type){
                    targ_type = new MetaType();
                    targ_type->setTypeUsagePattern(MetaType::VoidPattern);
                    targ_type->setTypeEntry(type);
                }
            }
            if(targ_type)
        ...
        if(MetaClass * iteratorClass = classes().findClass(iteratorType->qualifiedCppName(), MetaClassList::QualifiedCppName)){
            if(iteratorClass->typeAliasType()){
                ...
                if(iteratorClass->typeAliasType()->typeEntry()->isAlias()){
                    m_missing_aliases << MissingAlias(iteratorType, meta_type, m_current_class);
                    found = true;
                }else{
    ...
    }else if (meta_type->typeEntry()->isAlias()){
        m_missing_aliases << MissingAlias(meta_type->typeEntry(), meta_type, m_current_class);
    }
    return meta_type;
}
        ...
        if(missingAlias.aliasType->isIterator()){
            if(MetaClass * iteratorClass = classes().findClass(missingAlias.aliasType->qualifiedCppName(), MetaClassList::QualifiedCppName)){
                if(iteratorClass->typeAliasType()){
                    ...
                    missingAlias.meta_type->addIteratorInstantiation(typeAliasType);
                    found = true;
                }
            }
            if(!found && missingAlias.current_class && missingAlias.aliasType->isIterator()){
                const IteratorTypeEntry* iteratorType = static_cast<const IteratorTypeEntry*>(missingAlias.aliasType);
                if(MetaClass * iteratorClass = missingAlias.current_class->findIterator(iteratorType)){
                    missingAlias.meta_type->setIteratorInstantiations(iteratorClass->templateBaseClassInstantiations());
                    found = true;
                }
                if(missingAlias.current_class->templateBaseClass()){
            ...
            while(missingAlias.meta_type->typeEntry()->isAlias()){
                if(MetaClass * aliasClass = classes().findClass(missingAlias.meta_type->typeEntry()->qualifiedCppName(), MetaClassList::QualifiedCppName)){
                    if(const MetaType* typeAliasType = aliasClass->typeAliasType()){
                        if(static_cast<const AliasTypeEntry*>(missingAlias.meta_type->typeEntry())->getAsNativePointer()){
                            if(!typeAliasType->indirections().isEmpty())
                                const_cast<AliasTypeEntry*>(static_cast<const AliasTypeEntry*>(missingAlias.meta_type->typeEntry()))->setHasIndirections(true);
                            break;
                        }else{
                            missingAlias.meta_type->setTypeEntry(typeAliasType->typeEntry());
                            missingAlias.meta_type->setConstant(missingAlias.meta_type->isConstant() || typeAliasType->isConstant());
                            missingAlias.meta_type->setIndirections(QList<bool>{missingAlias.meta_type->indirections()} << typeAliasType->indirections());
                        }
                        MetaBuilder::decideUsagePattern(missingAlias.meta_type);
                    }else{
                        break;
                    }
                }else{
            ...
            if(!missingAlias.meta_type->typeEntry()->isAlias() || static_cast<const AliasTypeEntry*>(missingAlias.meta_type->typeEntry())->getAsNativePointer())
                found = true;
        }
        if(!found)
            missing_aliases << missingAlias;
        else
            MetaBuilder::decideUsagePattern(missingAlias.meta_type);
    }
    bool repeat = !missing_aliases.isEmpty() && m_missing_aliases.size()>missing_aliases.size();
    m_missing_aliases = missing_aliases;
    if(repeat){
        fixMissingIterator();
    }
}
...
void MetaBuilder::decideUsagePattern(MetaType *meta_type) {
    ...
    if (type->isPrimitive()) {
        if(meta_type->actualIndirections() == 0
            ...
            else if(type->qualifiedCppName()=="std::exception_ptr")
                meta_type->setTypeUsagePattern(MetaType::ExceptionPattern);
            else
    ...
    } else if (type->isAlias() && !static_cast<const AliasTypeEntry*>(type)->getAsNativePointer()) {
        meta_type->setTypeUsagePattern(MetaType::NativePointerPattern);
    } else if (type->isNativePointer()) {
        meta_type->setTypeUsagePattern(MetaType::NativePointerPattern);
    } else if (type->isAuto()) {
        meta_type->setTypeUsagePattern(MetaType::AutoPattern);
    } else if (type->isVoid()) {
        if(meta_type->indirections().isEmpty() && meta_type->getReferenceType()==MetaType::NoReference){
            meta_type->setTypeUsagePattern(MetaType::VoidPattern);
        }else{
            meta_type->setTypeUsagePattern(MetaType::NativePointerPattern);
        }
...
QString MetaBuilder::translateDefaultValue(const QString& defaultValueExpression, MetaType *type,
    ...
    if(type->isTemplateArgument()){
        if(type->indirections().isEmpty())
            ...
            if (expr == "0" || expr == "nullptr" || expr == "NULL" || expr == "null") {
                return "null";
            }
        }
    }
    ...
    if (type && type->typeEntry() && type->typeEntry()->isContainer()) {
        ...
        case ContainerTypeEntry::std_array:
            if(expr == "{}"){
                const ComplexTypeEntry* inst = cte->instantiations().values()[0];
                QString arrayType = inst->targetLangName();
...
void MetaBuilder::inheritHiddenBaseType(MetaClass *subclass, const MetaClass *hidden_base_class,
    ...
    for (auto l = qMin(template_types.size(), qMin(hidden_base_class->templateArguments().size(), info.arguments().size())), i = decltype(l)(0); i < l; ++i) {
        template_types_by_tvar.insert(hidden_base_class->templateArguments().at(i), template_types.at(i));
        template_types_by_name.insert(hidden_base_class->templateArguments().at(i)->name(), template_types.at(i));
    }

    {
        ...
        if(!implements.isEmpty()){
            for (QHash<const TypeEntry *,const MetaType *>::const_iterator ki = template_types_by_tvar.constBegin();
                 ki != template_types_by_tvar.constEnd(); ki++) {
    ...
    if(iteratorTypeEntry){
        newIteratorClass = instantiateIterator(iteratorTypeEntry, subclass, template_types, template_types_by_tvar);
    }
...
void MetaBuilder::setupTextStreamFunctions(MetaClass *cls) {
    if(cls->typeEntry()->isAddTextStreamFunctions()){
        FunctionModificationList mods = cls->typeEntry()->functionModifications();
        FunctionModification textStreamFunctionModification;
        for(auto l = mods.size(), i = decltype(l)(0); i<l; ++i){
            if(mods[i].signature=="QTextStreamFunction"){
                textStreamFunctionModification = mods[i];
                mods.removeAt(i);
                cls->typeEntry()->setFunctionModifications(mods);
                break;
            }
        }
        for(MetaFunction *f : std::as_const(m_textStreamFunctions)) {
            f = f->copy();
            *f -= MetaFunction::Static;
            *f -= MetaFunction::Private;
            *f -= MetaFunction::Protected;
            *f += MetaFunction::Public;
            f->setIsTextStreamFormat(true);
            f->setImplementingClass(cls);
            if(!f->declaringClass())
                f->setDeclaringClass(cls);
            if(f->arguments().size()==1 && f->arguments()[0]->type()->typeEntry()->qualifiedCppName()=="QTextStream"){
                FunctionModification mod;
                mod.signature = f->minimalSignature();
                ArgumentModification amod(1);
                amod.removed = true;
            ...
            MetaType* type;
            if((type = f->type())){
                type->setIndirections({});
                type->setInstantiations({});
            ...
            if(!textStreamFunctionModification.signature.isEmpty()){
                FunctionModification mod = textStreamFunctionModification;
                mod.signature = f->minimalSignature();
                cls->typeEntry()->addFunctionModification(mod);
            }
            cls->addFunction(f);
...
void MetaBuilder::setupConstructorAvailability(MetaClass *meta_class){
    ...
    if(meta_class && !meta_class->isNamespace()){
        ...
        if(meta_class->typeEntry()->isComplex()){
            const ComplexTypeEntry* centry = reinterpret_cast<const ComplexTypeEntry*>(meta_class->typeEntry());
            if(centry->getNotAssignable()){
                inheritingPublicDefaultAssignment = false;
            }
            if(centry->getNotMoveAssignable()){
                inheritingPublicMoveAssignment = false;
            }
            if(centry->getNotCloneable()){
                inheritingPublicCopyConstructor = false;
            }
        }
        if(meta_class->typeEntry()->hasPrivateConstructors()
                && !meta_class->typeEntry()->hasPublicDefaultConstructor()){
            inheritingPublicDefaultConstructor = false;
        }
        if(!meta_class->typeEntry()->hasPublicCopyConstructor()
            ...
            for(MetaField* field : meta_class->fields()){
                if(!field->type()->hasInstantiations()){
                    if(field->type()->typeEntry()->isComplex()){
                        const ComplexTypeEntry* centry = reinterpret_cast<const ComplexTypeEntry*>(field->type()->typeEntry());
                        if(!centry->hasPublicCopyConstructor()){
                            inheritingPublicCopyConstructor = false;
                            break;
                        }
                    }
        ...
        if(!meta_class->typeEntry()->hasPublicDefaultAssignment()
            ...
            for(MetaField* field : meta_class->fields()){
                if(!field->type()->hasInstantiations()){
                    if(field->type()->typeEntry()->isComplex()){
                        const ComplexTypeEntry* centry = reinterpret_cast<const ComplexTypeEntry*>(field->type()->typeEntry());
                        if(!centry->hasPublicDefaultAssignment()){
                            inheritingPublicDefaultAssignment = false;
                            break;
                        }
        ...
        if(!meta_class->typeEntry()->hasPublicMoveAssignment()
            ...
            for(MetaField* field : meta_class->fields()){
                if(!field->type()->hasInstantiations()){
                    if(field->type()->typeEntry()->isComplex()){
                        const ComplexTypeEntry* centry = reinterpret_cast<const ComplexTypeEntry*>(field->type()->typeEntry());
                        if(!centry->hasPublicMoveAssignment()){
                            inheritingPublicMoveAssignment = false;
                            break;
                        }
                    }else if(field->type()->typeEntry()->isSmartPointer()){
        ...
        if((inheritingPublicDefaultConstructor || !hasSuperClasses)
                && !meta_class->typeEntry()->hasPublicDefaultConstructor()
            ...
            if(!meta_class->baseClass()
                && !(meta_class->attributes() & MetaAttributes::IsUnion)
                && meta_class->interfaces().isEmpty()
                && meta_class->queryFunctions(MetaClass::Constructors).isEmpty()
                && meta_class->typeEntry()->hasFields()
                && !meta_class->typeEntry()->hasNonPublicFields()){
                FunctionModification mod;
                std::unique_ptr<MetaFunction> fieldsConstructor{new MetaFunction()};
                fieldsConstructor->setType(nullptr);
                QString name = meta_class->qualifiedCppName().split("::").last();
                auto idx = name.indexOf('<');
                if(idx>=0){
                    name = name.mid(0, idx);
                }
                fieldsConstructor->setOriginalName(name);
                fieldsConstructor->setName(meta_class->simpleName());
                fieldsConstructor->setFunctionType(MetaFunction::ConstructorFunction);
                *fieldsConstructor += MetaAttributes::Public;
                *fieldsConstructor += MetaAttributes::Native;
                *fieldsConstructor += MetaAttributes::Final;
                *fieldsConstructor += MetaAttributes::BracketCall;
                ...
                for(MetaField* field : meta_class->fields()){
                    if(!field->isStatic()){
                        ...
                        if(!fmod.modified_type.isEmpty()){
                            ...
                            amod.modified_java_type = fmod.modified_java_type;
                ...
                if(!arguments.isEmpty()){
                    fieldsConstructor->setArguments(arguments);
                    fieldsConstructor->setDeclaringClass(meta_class);
                    fieldsConstructor->setImplementingClass(meta_class);
                    fieldsConstructor->setOriginalSignature(fieldsConstructor->minimalSignature());
                    fieldsConstructor->setOriginalAttributes(fieldsConstructor->attributes());
                    if(!mod.argument_mods.isEmpty()){
                        mod.signature = fieldsConstructor->minimalSignature();
                        meta_class->typeEntry()->addFunctionModification(mod);
                    }
                    meta_class->addFunction(fieldsConstructor.release());
                    meta_class->typeEntry()->setHasPublicCopyConstructor();
                    meta_class->typeEntry()->setHasPublicDefaultAssignment();
                }
            }
        ...
        if(meta_class->typeEntry()->hasPublicCopyConstructor()
            ...
            for(MetaFunction * constructor : meta_class->queryFunctions(MetaClass::Constructors)){
                ...
                if(constructor->isConstructor()
                    ...
                    && !constructor->hasUnresolvedTemplateTypes()
