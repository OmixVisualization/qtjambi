...
void CppImplGenerator::write(QTextStream &s, const MetaFunctional *java_functional, int) {
    ...
    if(java_functional->isFunctionPointer() && java_functional->typeEntry()->getUsing().isEmpty()){
    ...
    if(!(java_functional->typeEntry()->codeGeneration() & TypeEntry::GenerateNoShell)){
            ...
            s << INDENT << "if (jmethodID method_id = javaMethod()) {" << Qt::endl;
            {
                ...
                s << INDENT << "if (JniEnvironment env{" << QString::number( 100*(java_functional->arguments().size()+2) ) << "}) {" << Qt::endl;
                {
                    ...
                    if(isMessageHandler){
                        ...
                        s << INDENT << (java_functional->isNoExcept() ? "QtJambiExceptionInhibitor" : "QtJambiExceptionHandler") << " __qj_exnhandler;" << Qt::endl;
                    }
                        ...
                        INDENTATION(INDENT)
                        {
                            ...
                            QString lines;
                            {
                                ...
                                for(const MetaArgument* argument : java_functional->arguments()) {
                                    writeOwnership(s, nullptr, java_functional, "__java_" + argument->indexedName(), "__qt_" + argument->indexedName(), argument->argumentIndex() + 1, nullptr, TS::ShellCode, "env", "__qtjambi_scope");
                                }
                                ...
                                if (function_type) {
                                    QString scopeName = java_functional->needsReturnScope() ? QStringLiteral(u"__qtjambi_return_scope") : QStringLiteral(u"__qtjambi_scope");
                                    ...
                                    writeConversionRule(s, TS::ShellCode, nullptr, java_functional, 0, "<invalid>", "<invalid>", NoOption, "env", "__qtjambi_scope");
                                }
                            }
                    ...
                    s << INDENT << "} QTJAMBI_CATCH(const JavaException& exn) {" << Qt::endl;
                    {
                        ...
                        if(isMessageHandler){
                            ...
                            s << INDENT << "__qj_exnhandler.handle(env, exn, \"" << java_functional->typeEntry()->qualifiedCppName() << "\");" << Qt::endl;
                        }
                    }
        ...
        s << "void " << shellClassName(java_functional) << "::getFunctional(JNIEnv *";
        if(java_functional->isFunctionPointer())
            s << "env";
        ...
        s << "jobject " << shellClassName(java_functional) << "::resolveFunctional(JNIEnv * env, const void* functionPtr, bool* ok)" << Qt::endl
          << "{" << Qt::endl;
        {
            INDENTATION(INDENT)
    ...
    s << "{" << Qt::endl;
    {
        ...
        s << INDENT << "QTJAMBI_TRY {" << Qt::endl;
        {
            ...
            if (java_functional->type()) {
                ...
                if(java_functional->type()->isPrimitive()
                        ...
                        s << INDENT << "if(jobject __java_this = " << shellClassName(implementor) << "::__shell()->getJavaObjectLocalRef(__jni_env)){" << Qt::endl;
                        {
                            INDENTATION(INDENT)
                            {
                                ...
                                QString lines;
                                {
                                    ...
                                    for (int i = 0; i < arguments.size(); ++i)
                                        writeOwnership(s, java_function, nullptr, "__java_" + arguments.at(i)->indexedName(), arguments.at(i)->indexedName(), i + 1, implementor, TS::ShellCode, "__jni_env", "__qtjambi_scope");
                                    ...
                                    if (has_function_type && !java_function->isSelfReturningFunction()) {
                                        QString scopeName = java_function->returnScopeIndex()>=0 ? QStringLiteral(u"__qtjambi_return_scope") : QStringLiteral(u"__qtjambi_scope");
                                    ...
                                    writeOwnership(s, java_function, nullptr, "this", "__java_this", -1, implementor, TS::ShellCode, "__jni_env", "__qtjambi_scope");
                                    writeOwnership(s, java_function, nullptr, "__java_return_value", "__qt_return_value", 0, implementor, TS::ShellCode, "__jni_env", "__qtjambi_scope");
                                ...
                                if(lines.contains(QStringLiteral(u"__qtjambi_scope"))){
...
void CppImplGenerator::writeConstructor(QTextStream &s, const MetaFunction *java_function,
    ...
    if(cls->isQObject() && !cls->isAbstract())
        s << " __qtjambi_has_derivedMetaObject";
    s << ", bool";
    if(!cls->isAbstract() && cls->hasVirtualDestructor())
    ...
    if(java_function){
        ...
        if(useArgumentAsArray || useArgumentAsBuffer){
            ...
            if(utilArgumentIndex>0){
                argumentTypeArrayLength = java_function->arguments().value(utilArgumentIndex-1, nullptr);
                if(!argumentTypeArrayLength || argumentTypeArrayLength->argumentIndex()+1!=utilArgumentIndex){
        ...
        if(useArgumentAsBuffer || useArgumentAsArray){
            ...
            if(utilArgumentIndex>0){
                argumentTypeArrayLength = java_functional->arguments().value(utilArgumentIndex-1, nullptr);
                if(!argumentTypeArrayLength || argumentTypeArrayLength->argumentIndex()+1!=utilArgumentIndex){
                    argumentTypeArrayLength = nullptr;
                    for(const MetaArgument *argument : java_functional->arguments()) {
                        if(argument->argumentIndex()+1==utilArgumentIndex){
...
bool CppImplGenerator::writeJavaToQt(QTextStream &s,
    ...
    if (java_type->isJObjectWrapper()) {
        ...
        if(argument_index>0){
            if(java_function){
                argumentName = java_function->arguments()[argument_index-1]->argumentName();
            }else if(java_functional){
                argumentName = java_functional->arguments()[argument_index-1]->argumentName();
            }
        }
...
bool CppImplGenerator::writeQtToJava(QTextStream &s,
    ...
    if (java_type->isArray()) { // jniTypeName(java_type)
        ...
        }else if(elementType->isPrimitive() && java_type->arrayElementCounts().size()==1){
            s << INDENT;
            if((option & DirectReturn) == DirectReturn){
                s << "return ";
            }else if((option & NoTmpVariable) == NoTmpVariable){
                            ...
                            if (resetObjectAfterUse){
                                ...
                                if (type->isContainer()){
                                    if(static_cast<const ContainerTypeEntry *>(type->typeEntry())->type()==ContainerTypeEntry::std_array){
                                        s << INDENT << "scope->addDeletion(new InvalidateArrayAfterUse(env, out.l));" << Qt::endl;
                                    }else{
                                        s << INDENT << "scope->addDeletion(new InvalidateContainerAfterUse(env, out.l));" << Qt::endl;
                                    }
