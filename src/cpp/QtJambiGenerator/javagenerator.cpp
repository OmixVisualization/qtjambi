...
QString JavaGenerator::translateType(const MetaType *java_type, const MetaClass *context, Option option) {
    ...
    if (!java_type) {
        ...
        if (java_type->hasNativeId() && (option & UseNativeIds)) {
            ...
            QString argument = translateType(instantiation, context, Option((option & ~EnumAsInts & ~UseNativeIds) | BoxedPrimitive | NoSuppressExports | VarArgsAsArray));
                    ...
                    if ((option & SkipTemplateParameters) == 0) {
                        ...
                        for (int i=0; i<args.size(); ++i) {
                            ...
                            if((option & IsReturnType) != IsReturnType
                                ...
                                s += "? extends ";
                            s += translateType(args.at(i), context, Option((option & (NoNullness | IsReturnType)) | BoxedPrimitive | NoQCollectionContainers | VarArgsAsArray));
                        }
                        s += '>';
                    }
                }else{
        ...
        if (has_return_type && (java_function->argumentReplaced(0).isEmpty() || java_function->argumentReplaced(0)==QStringLiteral(u"this"))) {
            if (needs_return_variable) {
                if (new_return_type.isEmpty())
                    s << translateType(return_type, java_function->implementingClass(), Option(IsReturnType | (m_nullness ? NoOption : NoNullness)));
                else
                    s << new_return_type;
...
void JavaGenerator::writeSignal(QTextStream &s, const MetaFunction *java_function) {
    ...
    if (java_function->isPrivateSignal()) {
        ...
        for (const MetaArgument* argument : qAsConst(arguments)) {
            ...
            if(!defaultValueExpression.isEmpty()){
                ...
                if (type.isEmpty()){
                    type = translateType(argument->type(), java_function->implementingClass(), java_function->isPrivateSignal() ? Option(BoxedPrimitive | VarArgsAsArray | IsReturnType | (m_nullness ? NoOption : NoNullness)) : Option(BoxedPrimitive | VarArgsAsArray | NoQCollectionContainers | IsReturnType | (m_nullness ? NoOption : NoNullness)));
                }else{
    ...
    if (sz > 0) {
        ...
        for (const MetaArgument* argument : qAsConst(arguments)) {
            ...
            QString boxedType;
            if (modifiedType.isEmpty()){
                boxedType = translateType(argument->type(), java_function->implementingClass(), java_function->isPrivateSignal() ? Option(BoxedPrimitive | VarArgsAsArray | IsReturnType | (m_nullness ? NoOption : NoNullness)) : Option(BoxedPrimitive | VarArgsAsArray | NoQCollectionContainers | IsReturnType | (m_nullness ? NoOption : NoNullness)));
            }else{
...
void JavaGenerator::writeMultiSignal(QTextStream &s, const MetaFunctionList& signalList){
      ...
      << INDENT << "public final class MultiSignal_" << signalName << " extends MultiSignal{" << Qt::endl;
    {
                ...
                for(MetaFunction* java_function : signalList){
                    ...
                    if (java_function->isPrivateSignal()) {
                        ...
                        for (int i = 0; i < sz; ++i) {
                            ...
                            if(!defaultValueExpression.isEmpty()){
                                ...
                                if (type.isEmpty()){
                                    type = translateType(arg->type(), java_function->implementingClass(), java_function->isPrivateSignal() ? Option(BoxedPrimitive | VarArgsAsArray | IsReturnType | (m_nullness ? NoOption : NoNullness)) : Option(BoxedPrimitive | VarArgsAsArray | NoQCollectionContainers | IsReturnType | (m_nullness ? NoOption : NoNullness)));
                                }else{
                                    registerPackage(type);
                                    type = type.replace(u'$', u'.');
                                }
                    ...
                    if (sz > 0) {
                        ...
                        for (int i = 0; i < sz; ++i) {
                            ...
                            QString unboxedType;
                            if (modifiedType.isEmpty()){
                                boxedType += translateType(arg->type(), java_function->implementingClass(), Option(BoxedPrimitive | VarArgsAsArray | IsReturnType | (m_nullness ? NoOption : NoNullness)));
                                QString type = translateType(arg->type(), java_function->implementingClass(), Option(NoNullness | VarArgsAsArray | IsReturnType));
        ...
        for(MetaFunction* java_function : signalList){
            ...
            for(int i = 0; i < java_function->arguments().size(); i++){
                QString type = translateType(java_function->arguments().at(i)->type(), java_function->implementingClass(), Option(NoNullness | VarArgsAsArray | IsReturnType));
                auto idx = type.indexOf(u'<');
                if(idx>0){
                    type = type.mid(0, idx);
                    if(type==QStringLiteral(u"io.qt.core.QList"))
...
void JavaGenerator::writeReferenceCount(QTextStream &s, const ReferenceCount &refCount,
    ...
    if(argumentIndex==-1){
        ...
        argument = arguments.at(argumentIndex-1);
...
void JavaGenerator::writeDeprecatedComment(QTextStream& commentStream, const MetaFunction *java_function){
    ...
    if(foundFun && !foundFun->isSignal()){
        ...
        commentStream << "@deprecated " << encodeHtml(java_function->deprecatedComment());
    }
}
    ...
    if(idx>0){
        ...
        if(elementType.contains(u',')){
            ...
            for(const QString& e : elements){
                elementType = QString(u"%1, %2").arg(elementType, toMetaTypeExpression(e));
            }
        }else{
            elementType = toMetaTypeExpression(elementType);
        }
        return QString(u"io.qt.core.QMetaType.fromType(%1.class%2)").arg(containerType, elementType);
    }
    return QString(u"io.qt.core.QMetaType.fromType(%1.class)").arg(elementType);
}
    ...
    for (const MetaArgument *arg : arguments){
        if(java_function->argumentRemoved(arg->argumentIndex() + 1)==ArgumentRemove_No
            ...
            if(arg->type()->isQSpan() && !java_function->isNoImplicitCalls(arg->argumentIndex()+1) && !functionIsNoImplicitArguments){
                ...
                QString element_type = translateType(instantiation, java_function->implementingClass(), Option((_option & ~EnumAsInts & ~UseNativeIds & ~BoxedPrimitive) | NoSuppressExports | VarArgsAsArray));
                QString nonnull;
                ...
                QString el = translateType(instantiation, java_function->implementingClass(),
                                    ...
                                    if(alternativeType.type==Replacement::RemoveArgument){
                                        ...
                                        if (java_type.isEmpty()){
                                            if(java_function->useArgumentAsBuffer(arg->argumentIndex()+1)){
                                                ...
                                                java_type = translateType(arg->type(), java_function->implementingClass(), Option(option | VarArgsAsArray | NoNullness));
                                            }
                                        }
                                        ...
                                        if(java_type==QStringLiteral(u"int")
                                        ...
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("ByteBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  pre, s, post, u'B');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("ShortBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  annotationFreeAlternativeType,
                                                                  pre, s, post, u'S');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("IntBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  annotationFreeAlternativeType,
                                                                  pre, s, post, u'I');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("LongBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  annotationFreeAlternativeType,
                                                                  pre, s, post, u'J');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("FloatBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  annotationFreeAlternativeType,
                                                                  pre, s, post, u'F');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("DoubleBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                                  ...
                                                                  annotationFreeAlternativeType,
                                                                  pre, s, post, u'D');
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("CharBuffer")
                                            ...
                                            writeIterableToBuffer(java_function,
                                                 ...
                                                 && annotationFreeAlternativeType==QStringLiteral(u"CharSequence")){
                                            s << arg->modifiedArgumentName() << "==null ? null : java.nio.CharBuffer.wrap(" << arg->modifiedArgumentName() << ")";
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("ByteBuffer") && annotationFreeAlternativeType==QStringLiteral(u"io.qt.core.QByteArray")){
                                            if(java_function->isConstructor()){
                                        ...
                                        }else if(java_type.startsWith("java.nio.") && java_type.endsWith("ByteBuffer") && annotationFreeAlternativeType==QStringLiteral(u"io.qt.core.QByteArrayView")){
                                            if(java_function->isConstructor()){
                                                s << arg->modifiedArgumentName() << ".data()";
                                            }else{
                                                pre << "__" << arg->modifiedArgumentName() << "_buffer = " << arg->modifiedArgumentName() << ".data();" << Qt::endl;
                                                s << "__" << arg->modifiedArgumentName() << "_buffer";
                                                post << "QtJambi_LibraryUtilities.internal.truncateBuffer(" << arg->modifiedArgumentName() << ", __" << arg->modifiedArgumentName() << "_buffer);" << Qt::endl;
                                            }
                                        }else if(java_type==QStringLiteral(u"byte[]")
                                            ...
                                            if(alternativeType.typeName.endsWith(QStringLiteral(u"@StrictNonNull[]"))
                                                ...
                                                if(java_type=="java.nio.Buffer"){
                                                    ...
                                                    s << arg->modifiedArgumentName() << "==null ? null : " << java_type.replace(u'$', u'.') << ".wrap(" << arg->modifiedArgumentName();
                                                }
                                            }
                                        ...
                                        }else if((java_type.startsWith("java.util.Collection<") || java_type=="java.util.Collection"
                                                  || java_type.startsWith("java.lang.Iterable<") || java_type=="java.lang.Iterable")
                                            ...
                                            if(annotationFreeAlternativeType.startsWith("java.nio.")
                                                     && annotationFreeAlternativeType.endsWith("Buffer")){
                                                s << "Buffer(";
                                            ...
                                            s << arg->modifiedArgumentName() << ")";
                                        }else if((java_type.startsWith("io.qt.core.QSpan<") || java_type=="io.qt.core.QSpan")){
                                            if(!arg->type()->indirections().isEmpty()){
                                                s << arg->modifiedArgumentName() << "==null ? null : ";
                                            }
                                            ...
                                            s << arg->modifiedArgumentName() << ")";
                                        }else if((java_type.startsWith("io.qt.core.QList<") || java_type=="io.qt.core.QList")
                                            ...
                                            }else if(annotationFreeAlternativeType.startsWith("java.nio.")
                                                     && annotationFreeAlternativeType.endsWith("Buffer")){
                                        ...
                                        }else if((java_type.startsWith("io.qt.core.QSet<") || java_type=="io.qt.core.QSet")
                                                 && annotationFreeAlternativeType.endsWith(QStringLiteral(u"[]"))){
                                            if(!arg->type()->indirections().isEmpty()){
                                                s << arg->modifiedArgumentName() << "==null ? null : ";
                                            }
                                            ...
                                            if(arg->type()->typeEntry()->isQVariant()){
                                                if(!annotationFreeAlternativeType.endsWith("[]") && !annotationFreeAlternativeType.contains("<")){
                                                    if(annotationFreeAlternativeType.startsWith("io.qt.")){
                                                        if(alternativeType.typeName.contains("@StrictNonNull")){
                                                            s << "java.util.Objects.<java.lang.Object>requireNonNull(" << arg->modifiedArgumentName() << ")";
                                                        }else if(alternativeType.typeName.contains("@Nullable")){
                                    ...
                                    if (modified_type.isEmpty()) {
                                        ...
                                        if(defaultExpr=="null")
                                            s << "(" << modified_type.replace(" @Nullable", "").replace(" @NonNull", "").replace(" @StrictNonNull", "")
                                                            .replace("@Nullable ", "").replace("@NonNull ", "").replace("@StrictNonNull ", "").replace(u'$', u'.') << ")";
                                    }
                    ...
                    if(precontent.isEmpty() && postcontent.isEmpty()){
                        ...
                        if (new_return_type != "void"
                            ...
                            if(!new_return_type.isEmpty()){
                                ...
                                s << INDENT << translateType(java_function->type(), java_function->implementingClass(), Option(option));
                            }
...
void JavaGenerator::write(QTextStream &s, const MetaClass *java_class, int nesting_level) {
    ...
    QString lines;
    {
        ...
        if(java_class->templateBaseClass() && java_class->templateBaseClass()->typeEntry()->isContainer()){
            ...
            case ContainerTypeEntry::MultiHashContainer:
            {
                ...
                for(const MetaType * instantiation : java_class->templateBaseClassInstantiations()){
                    ...
                    s << translateType(instantiation, java_class, Option(BoxedPrimitive | IsReturnType | (m_nullness ? NoOption : NoNullness)));
                    k++;
                }
                s << ">";
                isTemplate = true;
        ...
        if (iterableType) {
            ...
            s << "java.lang.Iterable<" << translateType(iterableType, java_class, Option(BoxedPrimitive | IsReturnType | (m_nullness ? NoOption : NoNullness)))<< ">";
        }
...

        {
            ...
            if (iterableType) {
                ...
                if(m_nullness)
                    s << "@NonNull ";
                s << "Iterator<" << translateType(iterableType, java_class, Option(BoxedPrimitive | IsReturnType | (m_nullness ? NoOption : NoNullness))) << "> iterator() {" << Qt::endl
...
void JavaGenerator::writeFunctionAttributes(QTextStream &s, const MetaFunction *java_function, int arg_count, const QMap<int,Replacement>& replacedArguments,
    ...
    if ((options & SkipReturnType) == 0) {
        ...
        if(java_function->type() && java_function->useArgumentAsBuffer(0)){
            ...
            MetaBuilder::decideUsagePattern(cpy.get());
            modified_type = translateType(cpy.get(), java_function->implementingClass(), Option((options & ~UseNativeIds) | NoNullness | IsReturnType));
            if(modified_type==QStringLiteral(u"int")){
            ...
            MetaBuilder::decideUsagePattern(cpy.get());
            modified_type = translateType(cpy.get(), java_function->implementingClass(), Option((options & ~UseNativeIds) | IsReturnType)).replace(u'$', u'.');
            modified_type += "[]";
        }else{
        ...
        if (modified_type.isEmpty()){
            ...
            returnType = translateType(java_function->type(), java_function->implementingClass(), Option(options | VarArgsAsArray | IsReturnType));
        }else{
