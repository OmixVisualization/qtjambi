...
```
(exchange `$VERSION` either by `6.8.8`, `6.10.2` or any other published version).
...
``` powershell
javac -cp qtjambi-6.8.8.jar Test.java
```
...
Make sure you are using the same Qt version and QtJambi version (the first and second version digit needs to be the same, e.g. 6.8 or 6.10).
...
``` powershell
java -cp qtjambi-6.8.8.jar;qtjambi-native-windows-x64-6.8.8.jar;. -Djava.library.path=C:\Qt\6.8.3\msvc2022_64\bin Test
```

On Linux it looks this way:

``` bash
java -cp qtjambi-6.8.8.jar:qtjambi-native-linux-x64-6.8.8.jar:. -Djava.library.path=<path to>/Qt/6.8.3/gcc_64/lib Test
```

On macOS you additionally need to use the start parameter -XstartOnFirstThread:

``` bash
java -cp qtjambi-6.8.8.jar:qtjambi-native-macos-6.8.8.jar:. -Djava.library.path=<path to>/Qt/6.8.3/macos/lib -XstartOnFirstThread Test
```
...
If you intend to use automatic module loading (`java -p <dir>`) you strictly need to place native components in `native` subfolder next to `qtjambi-6.8.8.jar`.
