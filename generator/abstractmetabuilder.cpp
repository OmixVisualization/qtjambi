...
void AbstractMetaBuilder::checkFunctionModifications() {
    ...
    foreach(TypeEntry *entry, entries) {
        ...
        foreach(FunctionModification modification, modifications) {
            ...
            AbstractMetaClass *clazz = m_meta_classes.findClass(centry->qualifiedCppName(), AbstractMetaClassList::QualifiedCppName);
            if (clazz == 0)
                continue;
...
bool AbstractMetaBuilder::build() {
    ...
    foreach(AbstractMetaClass *cls, m_meta_classes) {
        cls->fixPrivatePureVirtualFunctions();
    }
...
int AbstractMetaBuilder::figureOutEnumValue(const QString &origStringValue,
    ...
    // This block deals with "FooBar::Type(.....)" around the part we really want "....."
    {
        ...
        if (beg_pos >= 0) {
            ...
            tmp_stringValue.remove(0, beg_pos+1).trimmed();	// remove "FooBar::Type("
    ...
    for (int i = 0; i < stringValues.size(); ++i) {
...

        {
            ...
            if (beg_pos >= 0) {
                ...
                tmp_stringValue.remove(0, beg_pos+1).trimmed();	// remove "("
...
void AbstractMetaBuilder::traverseFunctions(ScopeModelItem scope_item, AbstractMetaClass *meta_class) {
    ...
    QSet<QString> privatePureVirtualFunctions;
    ...
    foreach(FunctionModelItem function, scope_item->functions()) {
        ...
        if (meta_function) {
            ...
            if(meta_function->isDestructor()){
                ...
                privatePureVirtualFunctions << meta_function->signature();
            }
...
//                  don't make public! just skip protected methods in the interface
    ...
    meta_class->setPrivatePureVirtualFunctions(privatePureVirtualFunctions);
}

bool AbstractMetaBuilder::setupTemplateInstantiations(AbstractMetaClass *meta_class){

    AbstractMetaFunctionList funcs = meta_class->functions();
    foreach(AbstractMetaFunction *func, funcs) {
        if(func->hasTemplateArgumentTypes()){
            ...
            if(!templateInstantiations.isEmpty()){
                foreach(TemplateInstantiation template_instantiation , templateInstantiations){
                    // duplicate
                    ...
                    if(func2->type()->typeUsagePattern()==AbstractMetaType::TemplateArgumentPattern){
                        ...
                        if(rtype){
                            ...
                            rtype->setReference(func2->type()->isReference());
                    ...
                    foreach(AbstractMetaArgument* arg, func2->arguments()){
                        if(arg->type()->typeUsagePattern()==AbstractMetaType::TemplateArgumentPattern){
                            ...
                            if(rtype){
                                ...
                                rtype->setReference(arg->type()->isReference());
                                decideUsagePattern(rtype);
                                arg->setType(rtype);
                            }
                        }
...
bool AbstractMetaBuilder::setupInheritance(AbstractMetaClass *meta_class) {
    ...
    for (int i = 0; i < base_classes.size(); ++i) {
        ...
        if (i != primary) {
            ...
            QString interface_name = InterfaceTypeEntry::interfaceName(base_class->fullName());
            AbstractMetaClass *iface = m_meta_classes.findClass(interface_name, AbstractMetaClassList::FullName);
            if (!iface) {
            ...
            if(iface->isInterface()){
                ...
                ReportHandler::warning(QString("declared interface for '%1' for class '%2' is not an interface type: '%3'")
                                       .arg(interface_name)
                                       .arg(meta_class->name())
                                       .arg(iface->fullName()));
//                meta_class->setBaseClass(iface);
            }
...
AbstractMetaFunction *AbstractMetaBuilder::traverseFunction(FunctionModelItem function_item) {
    ...
    TemplateParameterList templateParameters = function_item->templateParameters();
    int k=0;
    foreach(TemplateParameterModelItem p , templateParameters){
        if(!p->ownerClass()){
    ...
    if (function_name.startsWith('~')) {
        ...
        if(metaTemplateParameters.contains(function_type.qualifiedName().join("::"))){
            ...
            type->setReference(function_type.isReference());
            ok = true;
        }
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        ...
        if(metaTemplateParameters.contains(arg->type().qualifiedName().join("::"))){
            ...
            meta_type->setReference(arg->type().isReference());
            ok = true;
        }

        if(!ok){
            meta_type = translateType(arg->type(), &ok, QString("traverseField %1.%2 arg#%3").arg(class_name).arg(function_name).arg(i));
        }
        if (!meta_type || !ok) {
    ...
    *ok = true;
    ...
    if (typeInfo.arrays.size() > 0) {
        ...
        if (!array_of_unspecified_size) {
            ...
            if (!elementType || !ok) {
                if(prependScope)
                    qDebug() << "Something has happened when trying to resolve type " << qPrintable(newInfo.toString());
                return 0;
            }
            ...
            for (int i = typeInfo.arrays.size() - 1; i >= 0; --i) {
                ...
                if (!ok) {
                    if(prependScope)
                        qDebug() << "Something has happened when trying to read array initialization: " << qPrintable(s);
                    return 0;
                }
    ...
    if (meta_type->typeEntry()->isPointerContainer()) {
