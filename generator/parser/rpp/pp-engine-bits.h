        ...
        public:
            ...
            void file(std::string const &filename, _OutputIterator __result) {
                ...
                if (fp != nullptr) {
                    ...
                    env.log(QString("File '%1' not found!").arg(QString::fromStdString(filename)).toStdString());
                }
            }
            ...
            void file(FILE *fp, _OutputIterator result) {
                ...
                if (!file.open(fp, QIODevice::ReadOnly)) { //succeeds always
                    env.log(QString("Failed to open file %1 for read").arg(file.fileName()).toStdString());
                    exit(1);
                }
        ...
        private:
            ...
            _InputIterator handle_include(bool skip_current_path, _InputIterator first, _InputIterator last,
                ...
                if (fp) {
                    ...
                    std::replace(filename.begin(), filename.end(), '\\', '/');
#endif
                    static QList<QByteArray> ignoredFiles{
                            "type_traits",
                            "cstddef",
                            "utility",
                            "assert.h",
