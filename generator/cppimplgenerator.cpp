...
QString CppImplGenerator::default_return_statement_qt(const AbstractMetaType *java_type, Generator::Option options) {
    ...
    if(java_type->typeEntry()->isValue()){
        if(AbstractMetaClass* cls = m_classes.findClass(java_type->typeEntry()->name())){
            if(!cls->hasPublicStandardConstructor() && cls->hasConstructors() && cls->name()!="QItemSelection"){
                CustomFunction customFunction = reinterpret_cast<const ValueTypeEntry*>(java_type->typeEntry())->customConstructor(ComplexTypeEntry::DefaultConstructor);
    ...
    if (java_type->isTargetLangAnyStringView())
        return returnStr + "QAnyStringView()";
    if (java_type->isTargetLangUtf8StringView())
...
bool hasDeleter(const AbstractMetaClass *cls) {
    ...
    if((entry->isValue() && static_cast<const ValueTypeEntry*>(entry)->skipMetaTypeRegistration())
            && ((entry->codeGeneration() & TypeEntry::GenerateCode) != 0)){
        return lookupClassWithPublicDestructor(cls) != nullptr;
    }
    return false;
        ...
        s << INDENT << "QTJAMBI_TRY {" << Qt::endl;
        {
            ...
            if (java_class->type()) {
                ...
                if(java_class->type()->isPrimitive()
                        ...
                        && !java_class->hasConversionRule(TypeSystem::NativeCode, 0)
...
void CppImplGenerator::write(QTextStream &s, const AbstractMetaClass *java_class, int) {
    ...
    if(java_class->isQObject() && !java_class->isFinal() && standardConstructor && !java_class->hasUnimplmentablePureVirtualFunction()){
        ...
        s << "        jvalue arguments;" << Qt::endl
          ...
          << "        qtjambi_initialize_native_qobject(__jni_env, __jni_class, __jni_object, &__qt_construct_"
              ...
              << INDENT << "{" << Qt::endl;
            {
                  ...
                  << INDENT << "QTJAMBI_TRY {" << Qt::endl;
                {
                    ...
                    if(cls->qualifiedCppName()=="QString")
                        s << INDENT << "__java_return_value = qtjambi_from_qstring(__jni_env, *__qt_this);" << Qt::endl;
                    else
                        s << INDENT << "__java_return_value = qtjambi_from_qstring(__jni_env, QVariant(*__qt_this).toString());" << Qt::endl;
                    if(!cls->typeEntry()->ppCondition().isEmpty()){
                        s << "#else" << Qt::endl
                          << INDENT << "Q_UNUSED(";
                        if(cls->typeEntry()->isNativeIdBased()){
    ...
    if (!java_class->hasDefaultToStringFunction() && fun && !(qevent && core)) {
          ...
          << INDENT << "{" << Qt::endl;
        {
              ...
              << INDENT << "QTJAMBI_TRY {" << Qt::endl;
            {
                ...
                s << INDENT << "__java_return_value = qtjambi_from_qstring(__jni_env, res);" << Qt::endl;
                if(!java_class->typeEntry()->ppCondition().isEmpty()){
                    s << Qt::endl << "#else" << Qt::endl
                      << INDENT << "Q_UNUSED(";
                    if(java_class->typeEntry()->isNativeIdBased()){
...
void CppImplGenerator::writeCloneFunction(QTextStream &s, const AbstractMetaClass *java_class) {
      ...
      << INDENT << "{" << Qt::endl;
    {
          ...
          << INDENT << "QTJAMBI_TRY {" << Qt::endl;
        {
            ...
            s << INDENT << "qtjambi_check_resource(__jni_env, __qt_this);" << Qt::endl;
            if(java_class->typeEntry()->isValue() && static_cast<const ValueTypeEntry*>(java_class->typeEntry())->skipMetaTypeRegistration()){
                s << INDENT << "__java_return_value = qtjambi_from_object(__jni_env, new " << java_class->typeEntry()->qualifiedCppName() << "(*__qt_this), false, false);" << Qt::endl
                  << INDENT << "qtjambi_set_java_ownership(__jni_env, __java_return_value);" << Qt::endl;
            }else if(java_class->qualifiedCppName()=="QString"){
                s << INDENT << "__java_return_value = qtjambi_to_stringobject(__jni_env, *__qt_this);" << Qt::endl;
            }else if(java_class->qualifiedCppName()=="QChar"){
                s << INDENT << "__java_return_value = qtjambi_to_charobject(__jni_env, *__qt_this);" << Qt::endl;
            }else{
                    ...
                    if(function_type && (function_type->isPrimitive() || function_type->isEnum() || !function_type->indirections().isEmpty())){
                        ...
                        if(function_type->typeEntry()->isValue()){
                            if(AbstractMetaClass* cls = m_classes.findClass(function_type->typeEntry()->name())){
                                if(!cls->hasPublicStandardConstructor() && cls->hasConstructors() && cls->name()!="QItemSelection"){
                                    CustomFunction customFunction = reinterpret_cast<const ValueTypeEntry*>(function_type->typeEntry())->customConstructor(ComplexTypeEntry::DefaultConstructor);
                                    if(customFunction.hasCode()){
                                        finished = true;
        ...
        for(const AbstractMetaArgument *argument : arguments) {
            if (java_function->argumentRemoved(argument->argumentIndex() + 1)==ArgumentRemove_No) {
                ...
                if(java_function->argumentTypeArray(argument->argumentIndex() + 1)){
                    ...
                    if (modified_type.isEmpty()){
                        QString arg = jni_signature(argument->type(), format);
                        if(arg=="Lio_qt_core_QString_2")
                            arg = "Ljava_lang_CharSequence_2";
                        args += arg;
                    }else{
...
void CppImplGenerator::writeFinalFunctionArguments(QTextStream &s, const AbstractMetaFunction *java_function,
    ...
    while(addedArguments.first.contains(argumentCounter)){
        ...
        s << "," << Qt::endl
          << " " << jniName(argumentMod.modified_type) << " " << argumentMod.modified_name;
        ++argumentCounter;
    }
    for(const AbstractMetaArgument *argument : arguments) {
        if (java_function->argumentRemoved(argument->argumentIndex() + 1)==ArgumentRemove_No) {
        ...
        for(const AbstractMetaArgument *argument : arguments) {
            if(argument){
                ...
                case ArgumentRemove_No:
                    ...
                    if(jniType.isEmpty()){
                        if(typeReplaced.isEmpty() && java_function->argumentTypeArray(argument->argumentIndex()+1)){
                            QScopedPointer<AbstractMetaType> array(argument->type()->copy());
                            array->setConstant(false);
                            array->setReferenceType(AbstractMetaType::NoReference);
                            QScopedPointer<AbstractMetaType> cpy(argument->type()->copy());
...
void CppImplGenerator::writeFinalConstructor(QTextStream &s, const AbstractMetaFunction *java_function,
    ...
    s << INDENT << "QTJAMBI_TRY {" << Qt::endl;
    {
        ...
        QString argumentRef;
        if(arguments.size() + addedArguments.first.size() + addedArguments.second.size() == 0){
            argumentRef = "nullptr";
        }else if(arguments.size() + addedArguments.first.size() + addedArguments.second.size() == 1){
            ...
            argumentRef = "arguments";
            s << "        jvalue arguments[" << (arguments.size() + addedArguments.first.size() + addedArguments.second.size()) << "];" << Qt::endl;
            int argumentCounter = 1;
            while(addedArguments.first.contains(argumentCounter)){
                const ArgumentModification& argumentMod = addedArguments.first[argumentCounter];
                QString jniType = jniName(argumentMod.modified_type);
        ...
        if(java_class->isQObject()){
            s << "        qtjambi_initialize_native_qobject(__jni_env, __jni_class, __jni_object, &__qt_construct_"
                                                            << java_class->qualifiedCppName().replace("::", "_").replace("<", "_").replace(">", "_")
            ...
            if(const ContainerTypeEntry* ctype = java_class->findContainerSuperClass(&instantiations)){
                ...
                bool canRegister = (cls->typeEntry()->isValue() && !static_cast<const ValueTypeEntry*>(cls->typeEntry())->skipMetaTypeRegistration()) || (cls->typeEntry()->isInterface() && static_cast<const InterfaceTypeEntry*>(cls->typeEntry())->origin()->isValue());
                ...
                if(!canRegister && cls->typeEntry()->isValue() && !static_cast<const ValueTypeEntry*>(cls->typeEntry())->skipMetaTypeRegistration()){
                    canRegister = !cls->typeEntry()->customConstructor(ComplexTypeEntry::LegacyCopyConstructor).code().isEmpty()
                                    || !cls->typeEntry()->customConstructor(ComplexTypeEntry::CopyConstructor).code().isEmpty()
                                    || !cls->typeEntry()->customConstructor(ComplexTypeEntry::MoveConstructor).code().isEmpty()
                                    || !cls->typeEntry()->customConstructor(ComplexTypeEntry::DefaultConstructor).code().isEmpty()
                    ...
                    if(_java_type->typeEntry()->isPrimitive()){
                        if(_java_type->typeEntry()->targetLangName()=="int"){
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jintArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jintArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jlongArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jlongArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jshortArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jshortArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jbyteArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jbyteArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jcharArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jcharArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jfloatArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jfloatArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jdoubleArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jdoubleArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
                            ...
                            if(qtjambi_scope.isEmpty()){
                                ...
                                if((option & NoTmpVariable) != NoTmpVariable)
                                    s << "jbooleanArray ";
                                s  << tmp_out_name << " = qtjambi_array_cast<jbooleanArray>(" << __jni_env << ", ";
                                if((option & OptionalScope) == OptionalScope)
                                        s << "*";
                                s << qtjambi_scope << ", " << in_name << ", " << (argumentTypeArrayDeref ? "*" : "");
                                if(argumentTypeArrayLength)
...
bool CppImplGenerator::writeJavaToQt(QTextStream &s,
    ...
    if (java_type->isJObjectWrapper()) {
    ...
    } else if (java_type->isTargetLangLatin1StringView()) {
        if((option & DirectReturn) == DirectReturn){
            s << INDENT << "return ";
        }else if((option & NoTmpVariable) == NoTmpVariable){
            if(!qt_name.isEmpty())
        ...
        if((option & OptionalScope) == OptionalScope){
                ...
                s << INDENT << "if(" << qtjambi_scope << " && !" << qt_name << "){" << Qt::endl;
                {
                    ...
                    if(java_type->indirections().isEmpty()){
                        if(this->hasPublicDefaultConstructor(java_type) || java_type->isPrimitive() || java_type->isEnum()){
                            ...
                            s << "#else" << Qt::endl
                              << INDENT << "QMetaType metaType = QMetaType::fromType<" << qualified_class_name << ">();" << Qt::endl;
                            s << INDENT << "if(metaType.id()!=QMetaType::UnknownType){" << Qt::endl;
                            {
                                INDENTATION(INDENT)
                                s << INDENT << "void* _" << qt_name << " = metaType.create(nullptr);" << Qt::endl;
            ...
            if((option & DirectReturn) == DirectReturn){
                ...
                if(!done){
                    s << INDENT;
                    if(java_type->getReferenceType()==AbstractMetaType::RReference){
...
bool CppImplGenerator::writeQtToJava(QTextStream &s,
    ...
    if (java_type->isArray()) { // jniTypeName(java_type)
        ...
        if(java_type->isTargetLangString() && !java_type->typeEntry()->isQString()){
            s << INDENT;
            if((option & DirectReturn) == DirectReturn){
                s << "return ";
            }else if((option & NoTmpVariable) == NoTmpVariable){
            ...
            s << "qtjambi_to_stringobject(" << __jni_env << ", " << qt_name << ")";
        }else{
            s << INDENT;
            if((option & DirectReturn) == DirectReturn){
                s << "return ";
        ...
        if(java_type->typeEntry()->isChar()){
            ...
            s << "qtjambi_to_charobject(" << __jni_env << ", " << qt_name << ")";
        }
        ...
        if (java_type->isContainer()) {
            ...
            if(java_type->indirections().isEmpty() && java_type->getReferenceType()!=AbstractMetaType::NoReference)
            ...
            if (java_type->isQObject()) {
                ...
                if(static_cast<const ValueTypeEntry*>(java_type->typeEntry())->skipMetaTypeRegistration()){
                    if(java_type->getReferenceType()==AbstractMetaType::NoReference){
                        s << "[&]() -> jobject { jobject result = qtjambi_from_object(" << __jni_env << ", new " << java_type->typeEntry()->qualifiedCppName() << "(std::move(" << qt_name << ")), false, false); qtjambi_set_java_ownership(" << __jni_env << ", result); return result; }()";
                    }else{
                        s << "[&]() -> jobject { jobject result = qtjambi_from_object(" << __jni_env << ", new " << java_type->typeEntry()->qualifiedCppName() << "(" << qt_name << "), false, false); qtjambi_set_java_ownership(" << __jni_env << ", result); return result; }()";
                    }
                }else{
                    ...
                    if (!centry || centry->polymorphicIdValue().isEmpty()) {
                        if(centry && (centry->isInterface() || centry->designatedInterface())){
                            if(entry->isObject() && java_type->indirections().isEmpty() && java_type->getReferenceType()!=AbstractMetaType::NoReference)
                                s << "qtjambi_cast<jobject>(" << __jni_env << ", &" << qt_name << ")";
                            else
                                s << "qtjambi_cast<jobject>(" << __jni_env << ", " << qt_name << ")";
                        }else{
                            if(entry->qualifiedCppName()=="__GLsync"){
                                s << "qtjambi_from_object(" << __jni_env << ", static_cast<const void *>(" << (java_type->getReferenceType()==AbstractMetaType::Reference ? "&" : "") << qt_name << "), \"" << QString(package).replace(".", "/") << "/"
                                        << entry->lookupName() << "\", false, false)";
                            }else{
                                if(entry->isObject() && java_type->indirections().isEmpty() && java_type->getReferenceType()!=AbstractMetaType::NoReference)
                                    s << "qtjambi_cast<jobject>(" << __jni_env << ", &" << qt_name << ")";
                                else
            ...
            } else if(java_type->typeEntry()->qualifiedCppName()=="__GLsync"){
                s << "qtjambi_from_object(" << __jni_env << ", static_cast<const void *>(" << (java_type->getReferenceType()==AbstractMetaType::Reference ? "&" : "")
                  << qt_name << "), \"" << QString(java_type->typeEntry()->javaPackage()).replace(".", "/") << "/"
                        << java_type->typeEntry()->lookupName() << "\", false, false)";
            } else {
                if(java_type->typeEntry()->isObject() && java_type->indirections().isEmpty() && java_type->getReferenceType()!=AbstractMetaType::NoReference)
                    s << "qtjambi_cast<jobject>(" << __jni_env << ", &" << qt_name << ")";
                else
                    s << "qtjambi_cast<jobject>(" << __jni_env << ", " << qt_name << ")";
            }
...
void CppImplGenerator::writeFunctionCallArguments(QTextStream &s,
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        if(const AbstractMetaArgument *argument = arguments.at(i)){
            bool hasConversionRule = java_function->hasConversionRule(TypeSystem::NativeCode, argument->argumentIndex() + 1);
            if ((options & SkipRemovedArguments) == SkipRemovedArguments
...
void CppImplGenerator::writeExtraIncludes(QTextStream &s, const AbstractMetaClass *java_class, QSet<QString>& dedupe, bool skipQtJambi) {
    IncludeList includes = java_class->typeEntry()->extraIncludes();
    if(java_class->typeEntry()->designatedInterface()){
        includes << java_class->typeEntry()->designatedInterface()->extraIncludes();
    }
...
void CppImplGenerator::writeExtraIncludes(QTextStream &s, const AbstractMetaFunctional *java_class, QSet<QString>& dedupe, bool skipQtJambi) {
    IncludeList includes = java_class->typeEntry()->extraIncludes();
        ...
        s << INDENT << "infos << ParameterInfo(" << Qt::endl;
        {
            INDENTATION(INDENT)
            {
                INDENTATION(INDENT)
                {
                    ...
                    s << INDENT << "\"";
                    if(function->argumentTypeArray(index)){
                    ...
                    }else if(function->argumentTypeBuffer(index)){
                        if(type->typeEntry()->isPrimitive()){
                            if(type->typeEntry()->targetLangName()=="int"){
...
void CppImplGenerator::writeMetaInfo(QTextStream &s, const AbstractMetaClass *cls,
    ...
    QString code;
    {
...

        {
            ...
            if(!cls->isNamespace()
                    && !cls->isFake()
                    && cls->typeEntry()!=TypeDatabase::instance()->qvariantType()
                    && cls->typeEntry()!=TypeDatabase::instance()->qcharType()
                    && cls->typeEntry()!=TypeDatabase::instance()->qstringType()){
                bool isInterface = false;
                        ...
                        for(const AbstractMetaFunction *function : cls->functions()) {
                            if (function->isConstructor()
                                ...
                                for(int i=0; i<defaultArgumentOverloads.size(); ++i){
                                    ...
                                    while(addedArguments.first.contains(argumentCounter)){
                                        const ArgumentModification& argumentMod = addedArguments.first[argumentCounter];
                                        s2 << jni_signature(argumentMod.modified_type, JNISignatureFormat(NoModification | SlashesAndStuff));
                                        ++argumentCounter;
                                    }
                            ...
                            if(signalsInTargetLang.size()==0){
                                ...
                                for(const QString& key : signalNames){
                                    ...
                                    for (int i = 0; i < list.size(); ++i) {
                                        ...
                                        if(f->isRemovedFrom(cls, TypeSystem::NativeCode))
                                            continue;
                                        AbstractMetaArgumentList arguments;
...

                        {
                            INDENTATION(INDENT)
                            {
                                ...
                                s << INDENT << "[](const QMetaMethod& method, QList<ParameterInfo>& infos) {" << Qt::endl;
                                {
                                    ...
                                    for (int i = 0; i < signalsInTargetLang.size(); ++i) {
                                        const AbstractMetaFunction *signal = signalsInTargetLang.at(i);
                                        if(signal->isRemovedFrom(cls, TypeSystem::NativeCode))
                                            continue;
                                        QStringList pps = getFunctionPPConditions(signal);
                                        if(!signal->declaringClass()->typeEntry()->ppCondition().isEmpty() && !pps.contains(signal->declaringClass()->typeEntry()->ppCondition())){
                                            if(pps.isEmpty() || !signal->declaringClass()->typeEntry()->ppCondition().contains("|"))
                ...
                if (!metaType.isValid()){
                    ...
                    if(!emittedRegisterType_5 || !emittedRegisterType_6) {
                        ...
                        bool canRegister = (entry->isValue() && !static_cast<const ValueTypeEntry*>(entry)->skipMetaTypeRegistration()) || (entry->isInterface() && static_cast<const InterfaceTypeEntry*>(entry)->origin()->isValue());
                        if (canRegister) {
                            ...
                            if (!hasDefaultConstructor && !emittedRegisterType_5 && !emittedRegisterType_6) {
                ...
                for(AbstractMetaFunction * function : cls->functions()){
                    if (
                        !function->isEmptyFunction()
                        && !function->hasTemplateArgumentTypes()
                        && !function->isRemovedFromAllLanguages(cls)
                    ){
                        QList<const AbstractMetaType*> types;
                        if(!function->isConstructor() && function->type())
                            types << function->type();
                        for(AbstractMetaArgument* arg : function->arguments()){
                            types << arg->type();
                        }
                        while(!types.isEmpty()){
                            const AbstractMetaType* type = types.takeFirst();
